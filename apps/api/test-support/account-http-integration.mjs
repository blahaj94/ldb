...
import { accountFixture, withAccountApp, accountRequest, rawAccountRequest, expectAccountError, snapshot } from './account-http-fixtures.mjs'
  ...
  await withAccountApp(f, async (base) => {
    for (const headers of [
      ...
      const response = await rawAccountRequest(base, {
        method: 'GET', path: `/me?accessToken=${f.token.accessToken}&userId=${f.initial.user.id}`, headers,
      })
    ...
    for (const nickname of [null, 1, '\ta', 'a\n', '\ud800', '\udc00', '\u0085a', 'a\u2028', '', 'a'.repeat(21)]) {
      await expectAccountError(await accountRequest(base, f, 'PATCH', { nickname }), 400, 'INVALID_NICKNAME')
    }
  })
...
async function boundary(source, phase, boundaryKind, method) {
  ...
  try {
    await withAccountApp(f, async (base) => {
      ...
      if (shouldReject) await expectAccountError(response, 401, 'AUTHENTICATION_REQUIRED')
  ...
  if (isAdmission) assert.deepEqual(after, admittedBefore)
  if (!isAdmission) assert(after.session.last_active_at > before.session.last_active_at)
}
...
async function realLockExpiry(source, table, method, kind) {
  ...
  if (isIdleBoundary) await setDeadline(source, f.initial.session.id, deadline)
  else f.token = await f.deps.issueAccessJwt({
    userId: f.initial.user.id, sessionId: f.initial.session.id,
    issuedAt: f.now.getTime() / 1000, idleDeadline: deadline.getTime() / 1000,
  })
  const before = await snapshot(source, f)
  await withAccountApp(f, async (base) => {
    await withLock(source, table, f, async ({ runner, pid, unlock }) => {
      ...
      const restore = instrument(source, {
        query: async ({ sql, query, run }) => {
          ...
          if (isStart) started.resolve((await query('SELECT pg_backend_pid() AS pid'))[0].pid)
          return result
        },
      })
      let pending
...
async function removalBeforeAdmission(source, kind, method) {
  ...
  await withAccountApp(f, async (base) => {
    ...
    const restore = instrument(source, {
      query: async ({ sql, query, run }) => {
        ...
        if (isStart) {
          ...
          if (hasWaiter) waiterStarted.resolve()
        }
        return result
      },
    ...
    try {
      ...
      if (isLogout) assert.equal(removed.value.status, 204)
  ...
  try {
    await withAccountApp(f, async (base) => {
      ...
      try {
        ...
        } else await source.query('DELETE FROM users WHERE id=$1', [f.initial.user.id])
        ...
        if (isLogout) {
          assert.equal(after.user.nickname, before.user.nickname)
          assert.equal(after.session.last_active_at.getTime(), admitted.session.last_active_at.getTime())
...
async function databaseFailure(source, phase, applied, method) {
  ...
  try {
    await withAccountApp(f, async (base) => {
      const body = await expectAccountError(await accountRequest(base, f, method), 503, 'AUTH_UNAVAILABLE')
      assert.equal(body.error.message, '현재 계정 기능을 이용할 수 없습니다. 잠시 후 다시 시도해 주세요.')
    })
  } finally {
    restore()
  ...
  for (const value of [f.token.accessToken, f.initial.refreshToken, f.identity.subject, f.initial.user.id, '변경 이름', 'private SQL']) {
...
export async function assertAccountHttpIntegration(source, mark) {
  const cases = [
    ...
    ...['GET', 'PATCH'].flatMap((method) => [
      ...['user', 'session', 'owner', 'revoked', 'idle'].map((kind) => [`${method} inaccessible ${kind}`, () => inaccessible(source, kind, method)]),
      ...['jwt', 'idle'].map((kind) => [`${method} exact admission ${kind} expiry`, () => boundary(source, 'admission', kind, method)]),
      [`${method} function elapsed JWT preserves admitted request`, () => boundary(source, 'function', 'jwt', method)],
      [`${method} function exact idle rejects while preserving activity`, () => boundary(source, 'function', 'idle', method)],
      ...['users', 'auth_sessions'].flatMap((table) => ['jwt', 'idle'].map((kind) => [
        `${method} real ${table} lock crosses ${kind} deadline`, () => realLockExpiry(source, table, method, kind),
      ])),
    ...
    ['nickname write failure preserves committed activity', () => databaseFailure(source, 'write', false, 'PATCH')],
  ]
