...
export const DOMAIN_TABLES = ['auth_login_requests', 'auth_refresh_tokens', 'auth_sessions', 'users']
...
export async function waitForAuthenticatedReadiness(createDataSource, configuration, timeoutMs = 20_000) {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      if (remainingMs > 0) await delay(Math.min(125, remainingMs))
    }
  }
...
const expectedIndexes = [
  ...
  ['auth_login_requests', 'uq_auth_login_requests_exchange_code_hash', ['exchange_code_hash'], true, null],
  ['auth_login_requests', 'uq_auth_login_requests_launch_ticket_hash', ['launch_ticket_hash'], true, null],
  ...
  ['auth_refresh_tokens', 'uq_auth_refresh_tokens_unconsumed_session', ['session_id'], true, '(consumed_at IS NULL)'],
...
const expectedForeignKeys = [
  ['auth_refresh_tokens', 'fk_auth_refresh_tokens_session', ['session_id'], 'auth_sessions', ['id'], 'c'],
...
export async function assertSchema(dataSource, mark = () => undefined, migrationNames = ['InitialAuthSchema1788600000000']) {
  ...
  const constraintDifference = Math.max(actualConstraints.length, expectedConstraints.length)
    ? Array.from({ length: Math.max(actualConstraints.length, expectedConstraints.length) }).findIndex(
        (_, index) => actualConstraints[index] !== expectedConstraints[index],
      )
    : -1
  if (constraintDifference >= 0) {
    mark(`constraints ${expectedConstraints[constraintDifference] ?? 'end'} ${actualConstraints[constraintDifference] ?? 'end'}`)
  }
  ...
  assert.deepEqual(
    snapshot.indexes
      ...
      .map(({ table_name, index_name, columns, is_unique, predicate }) =>
        [table_name, index_name, columns, is_unique, predicate],
      ),
    expectedIndexes,
  )
  mark('foreign keys')
  assert.deepEqual(
    snapshot.foreignKeys.map(
      ({ source_table, constraint_name, source_columns, target_table, target_columns, delete_action }) =>
        [source_table, constraint_name, source_columns, target_table, target_columns, delete_action],
    ),
    expectedForeignKeys,
  )
...
const loginColumns = [
  'id', 'purpose', 'provider', 'client_id', 'provider_config_version', 'return_target_id',
  'created_at', 'expires_at', 'status', 'code_challenge', 'method', 'launch_ticket_hash',
  'state_hash', 'browser_binding_hash', 'oidc_nonce_hash', 'provider_pkce_ciphertext',
  'provider_pkce_iv', 'provider_pkce_tag', 'provider_pkce_key_id', 'verified_subject',
...
function loginRequest(status, id, overrides = {}) {
  ...
  const stateFields = {
    ...
    browser_started: {
      code_challenge: 'challenge', method: 'S256', state_hash: hash(12),
      browser_binding_hash: hash(13), oidc_nonce_hash: hash(14), provider_pkce_ciphertext: hash(15),
      provider_pkce_iv: Buffer.alloc(12, 16), provider_pkce_tag: Buffer.alloc(16, 17),
      provider_pkce_key_id: 'key-v1',
    },
    processing: {
      code_challenge: 'challenge', method: 'S256', state_hash: hash(18),
      browser_binding_hash: hash(19), oidc_nonce_hash: hash(20), provider_pkce_ciphertext: hash(21),
      provider_pkce_iv: Buffer.alloc(12, 22), provider_pkce_tag: Buffer.alloc(16, 23),
      provider_pkce_key_id: 'key-v1',
    },
    exchange_ready: {
      code_challenge: 'challenge', method: 'S256', verified_subject: 'subject',
      exchange_code_hash: hash(24), code_expires_at: later,
    },
...
export async function assertConstraintBehavior(dataSource) {
  ...
  await rejectConstraint(dataSource, 'fk_auth_sessions_user', (queryRunner) =>
    queryRunner.query(
      'INSERT INTO "auth_sessions" (id, user_id, created_at, last_active_at) VALUES ($1, $2, $3, $4)',
      ['20000000-0000-4000-8000-000000000002', '10000000-0000-4000-8000-000000009999', createdAt, later],
    ),
  )
  ...
  const validRequests = [
    ...
    loginRequest('exchange_ready', '30000000-0000-4000-8000-000000000004', {
      state_hash: hash(30), browser_binding_hash: hash(31), oidc_nonce_hash: hash(32),
      provider_pkce_ciphertext: hash(33), provider_pkce_iv: Buffer.alloc(12, 34),
      provider_pkce_tag: Buffer.alloc(16, 35), provider_pkce_key_id: 'retained-key',
    }),
    loginRequest('consumed', '30000000-0000-4000-8000-000000000005'),
    loginRequest('failed', '30000000-0000-4000-8000-000000000006'),
  ]
  for (const request of validRequests) await insertLogin(dataSource, request)

  const invalidRequests = [
    ['ck_auth_login_requests_status', loginRequest('created', '30000000-0000-4000-8000-000000000010', { status: 'unknown' })],
    ['ck_auth_login_requests_created_fields', loginRequest('created', '30000000-0000-4000-8000-000000000011', { method: null })],
    ['ck_auth_login_requests_browser_started_fields', loginRequest('browser_started', '30000000-0000-4000-8000-000000000012', { launch_ticket_hash: hash(40) })],
    ['ck_auth_login_requests_processing_fields', loginRequest('processing', '30000000-0000-4000-8000-000000000013', { verified_subject: 'too-early' })],
    ['ck_auth_login_requests_exchange_ready_fields', loginRequest('exchange_ready', '30000000-0000-4000-8000-000000000014', { method: null })],
    ['ck_auth_login_requests_consumed_fields', loginRequest('consumed', '30000000-0000-4000-8000-000000000015', { code_challenge: 'retained' })],
    ['ck_auth_login_requests_failed_fields', loginRequest('failed', '30000000-0000-4000-8000-000000000016', { verified_subject: 'retained' })],
    ['ck_auth_login_requests_pkce_fields', loginRequest('exchange_ready', '30000000-0000-4000-8000-000000000017', { provider_pkce_ciphertext: hash(41) })],
    ['ck_auth_login_requests_expiry', loginRequest('created', '30000000-0000-4000-8000-000000000018', { expires_at: createdAt })],
    ['ck_auth_login_requests_code_deadline', loginRequest('exchange_ready', '30000000-0000-4000-8000-000000000019', { code_expires_at: new Date('2026-09-05T00:11:00Z') })],
    ['ck_auth_login_requests_launch_hash_length', loginRequest('created', '30000000-0000-4000-8000-000000000020', { launch_ticket_hash: Buffer.alloc(31) })],
    ['ck_auth_login_requests_state_hash_length', loginRequest('exchange_ready', '30000000-0000-4000-8000-000000000021', { state_hash: Buffer.alloc(31) })],
    ['ck_auth_login_requests_browser_hash_length', loginRequest('exchange_ready', '30000000-0000-4000-8000-000000000022', { browser_binding_hash: Buffer.alloc(31) })],
    ['ck_auth_login_requests_nonce_hash_length', loginRequest('exchange_ready', '30000000-0000-4000-8000-000000000023', { oidc_nonce_hash: Buffer.alloc(31) })],
    ['ck_auth_login_requests_exchange_hash_length', loginRequest('exchange_ready', '30000000-0000-4000-8000-000000000024', { exchange_code_hash: Buffer.alloc(31) })],
    ['ck_auth_login_requests_config_nonempty', loginRequest('created', '30000000-0000-4000-8000-000000000025', { provider_config_version: '' })],
    ['ck_auth_login_requests_return_target_nonempty', loginRequest('created', '30000000-0000-4000-8000-000000000026', { return_target_id: '' })],
    ['ck_auth_login_requests_browser_started_fields', loginRequest('browser_started', '30000000-0000-4000-8000-000000000030', { method: null })],
    ['ck_auth_login_requests_processing_fields', loginRequest('processing', '30000000-0000-4000-8000-000000000031', { method: null })],
    ['ck_auth_login_requests_exchange_ready_fields', loginRequest('exchange_ready', '30000000-0000-4000-8000-000000000032', { verified_subject: null })],
    ['ck_auth_login_requests_consumed_fields', loginRequest('consumed', '30000000-0000-4000-8000-000000000033', { consumed_at: null })],
  ]
  for (const [constraint, request] of invalidRequests) {
    await rejectConstraint(dataSource, constraint, (queryRunner) => insertLogin(queryRunner, request))
  }
  ...
  await rejectConstraint(dataSource, 'uq_auth_login_requests_state_hash', (queryRunner) =>
    insertLogin(queryRunner, loginRequest('exchange_ready', '30000000-0000-4000-8000-000000000028', { state_hash: hash(12) })),
  )
