...
export async function readProviderJson(
  ...
  try {
    ...
    try {
      ...
      body = providerResponse.json().catch(() => {
        throw new LoginFailure(LOGIN_ERRORS.PROVIDER)
      })
    } catch {
      throw new LoginFailure(LOGIN_ERRORS.PROVIDER)
...
export async function exchangeGoogleCode(
  ...
  try {
    let signal: AbortSignal
    {
      ...
      try {
        secretResolution = Promise.resolve(
          resolveSecret({
    ...
    try {
      tokenRequest = fetchGoogle(registration.tokenEndpoint, request).catch(() => {
        throw new LoginFailure(LOGIN_ERRORS.PROVIDER)
      })
    } catch {
      throw new LoginFailure(LOGIN_ERRORS.PROVIDER)
    ...
    if (hasForm) {
      for (const key of [...pendingForm.keys()]) {
        pendingForm.delete(key)
      }
    }
    delete request.body
    const pendingResponse = response
