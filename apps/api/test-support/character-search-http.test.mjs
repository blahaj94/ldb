...
async function withSearchBoundary(operation, { validJwt = true, apiKey = 'synthetic-search-key' } = {}) {
  ...
  const deps = {
    ...
    async verifyAccessJwt() {
      ...
      if (!validJwt) throw new Error('private verifier canary')
      return principal
    },
...
function expectSearchError(response, status, code, message) {
  ...
  assert.doesNotMatch(response.body, /synthetic-search-key|private verifier canary|stack|<!doctype/i)
}
...
test('search requires one exact Bearer header before invalid raw query or configuration', async () => {
  await withSearchBoundary(async (base, calls) => {
    for (const headers of [
      ...
      { authorization: 'Bearer' }, { authorization: 'Bearer first second' },
...
test('search original URL rejects malformed structure and strict UTF-8 before configuration', async () => {
  const invalidQueries = [
    '', 'characterName=', 'characterName=a', `characterName=${'a'.repeat(13)}`,
    'characterName=%20ab', 'characterName=ab+',
    'characterName=ab&characterName=cd', 'characterName=ab&%63haracterName=cd',
    'characterName=ab&limit=1&%6Cimit=2', 'characterName=ab&serverId=all&serverId=cain',
    'characterName=ab&limit[]=1', 'characterName=ab&limit%5B0%5D=1',
    'characterName[x]=ab', 'characterName=ab&unknown=x',
    'characterName=ab&&limit=1', 'characterName=ab&',
    'characterName=%', 'characterName=%GG', 'characterName=%C0%AF',
    'characterName=%ED%A0%80', 'characterName=%F4%90%80%80', 'characterName=%FF',
    'characterName=ab&serverId=', 'characterName=ab&serverId=CAIN',
    'characterName=ab&serverId=future-server', 'characterName=ab&limit=',
    'characterName=ab&limit=0', 'characterName=ab&limit=201',
    'characterName=ab&limit=-1', 'characterName=ab&limit=1.0',
    'characterName=ab&limit=1e2', 'characterName=ab&limit=%EF%BC%91',
  ]
  await withSearchBoundary(async (base, calls) => {
...
test('search valid raw query reaches configuration failure without database or upstream', async () => {
  await withSearchBoundary(async (base, calls) => {
    ...
    expectSearchError(response, 500, 'INTERNAL_SERVER_ERROR', '서버 오류로 검색을 처리하지 못했습니다.')
    assert.equal(calls.verification, 1)
  }, { apiKey: '' })
})
