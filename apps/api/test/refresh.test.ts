...
test('refresh validates canonical 32 decoded bytes before any database work', async () => {
  ...
  for (const value of [null, {}, 1, '', canonical + '=', ' ' + canonical, canonical + '\n',
    '+'.repeat(43), '/'.repeat(43), noncanonical, randomBytes(31).toString('base64url'),
    randomBytes(33).toString('base64url'), digest(bytes), { refreshToken: canonical, sessionId: randomUUID() }]) {
    const f = fixture()
...
test('rotation hashes decoded bytes, locks in order and returns only after commit', async () => {
  ...
  assert.deepEqual(f.inserted, [{ tokenHash: digest(bytes), sessionId: f.session.id, issuedAt: time, consumedAt: null }])
  assert.equal(f.token.consumedAt, time)
  assert.equal(f.session.lastActiveAt, time)
  assert.deepEqual(f.events, ['begin', 'refresh-hint', 'session-hint', 'user-lock', 'session-lock',
    'refresh-lock', 'fresh-time', 'sign', 'consume', 'insert', 'commit'])
})
...
test('unknown, removed and stale ownership hints cannot select or revoke a session', async () => {
  const { rotateRefresh } = await load()
  for (const scenario of ['hintMissing', 'userMissing', 'sessionMissing', 'tokenMissing', 'session-owner', 'token-owner']) {
    const f = fixture()
    f.token.consumedAt = time
    if (scenario === 'session-owner') f.state.beforeLockedRead = () => { f.session.userId = randomUUID() }
    else if (scenario === 'token-owner') f.state.beforeLockedRead = () => { f.token.sessionId = randomUUID() }
    else Object.assign(f.state, { [scenario]: true })
    await failure(rotateRefresh(f.deps, f.raw), 'AUTHENTICATION_REQUIRED')
...
test('signing, entropy, insert and commit failures sanitize and rollback without retry', async () => {
  ...
  for (const scenario of ['sign', 'entropy', 'insert', 'commit']) {
    const f = fixture()
    const explode = () => { throw new Error('private detail') }
    if (scenario === 'sign') f.deps.issueAccessJwt = explode
    if (scenario === 'insert') f.state.beforeInsert = explode
    if (scenario === 'commit') f.state.beforeCommit = explode
    await failure(rotateRefreshForTest(f.deps, f.raw, scenario === 'entropy' ? explode : randomBytes),
