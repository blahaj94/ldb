...
try {
  ...
  assert.throws(() => new LoginRegistry(invalidConfig), (error) => {
  ...
  app = await createLoginHttpApp({
    create: fail, authorize: fail,
    exchange: () => successful ? { accessToken: 'credential-canary', refreshToken: 'credential-canary' } : fail(),
    callback: () => successful ? { returnUrl: `ldb-test://login/complete?code=${opaque()}`, cookie: '__Host-test=; Max-Age=0; Secure; HttpOnly; Path=/' } : fail(),
  })
  ...
  for (const [media, expected] of [['application/json', 413], ['text/plain', 415]]) {
    const rejected = await fetch(`${base}/auth/exchange`, {
      method: 'POST', headers: { 'content-type': media }, body: 'credential-canary'.repeat(1100),
    })
    assert.equal(rejected.status, expected)
    assert.doesNotMatch(await rejected.text(), /credential-canary/)
  }
  successful = true
  const completion = await fetch(`${base}/auth/callback/google?state=${opaque()}&code=credential-canary`)
  assert.equal(completion.status, 200)
  assert.match(await completion.text(), /ldb-test:/)
  const tokens = await fetch(`${base}/auth/exchange`, {
    method: 'POST', headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ requestId: '00000000-0000-4000-8000-000000000000', clientId: 'desktop', code: opaque(), codeVerifier: opaque() }),
  })
