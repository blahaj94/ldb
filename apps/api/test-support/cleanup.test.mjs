...
import { checkedAt, cleanupFixture, idleMilliseconds, request, session } from './cleanup-fixtures.mjs'

const load = () => import('../dist/auth/cleanup/index.js')

for (const [name, patch, expected] of [
  ['active', {}, 0],
  ['one second before idle deadline', { lastActiveAt: new Date(checkedAt.getTime() - idleMilliseconds + 1000) }, 0],
  ['exact idle deadline', { lastActiveAt: new Date(checkedAt.getTime() - idleMilliseconds) }, 1],
  ['after idle deadline', { lastActiveAt: new Date(checkedAt.getTime() - idleMilliseconds - 1000) }, 1],
  ...
  for (const change of ['activity', 'ownership', 'removed']) {
    ...
    const f = cleanupFixture({
      ...
      beforeLock: (rows) => {
        ...
        if (isActivity) row.lastActiveAt = checkedAt
        else if (isOwnership) row.userId = session().userId
        else rows.pop()
      },
    })
...
test('empty cleanup succeeds without owning or closing caller DataSource', async () => {
  const { cleanupAuthentication } = await load()
  const f = cleanupFixture()
  assert.deepEqual(await cleanupAuthentication(f.source), { sessionsDeleted: 0, loginRequestsDeleted: 0 })
  assert.deepEqual(await cleanupAuthentication(f.source), { sessionsDeleted: 0, loginRequestsDeleted: 0 })
...
test('later transaction failure cannot report success or erase earlier confirmed commits', async () => {
  ...
  const f = cleanupFixture({
    ...
    beforeCommit: (number) => {
      ...
      if (shouldFail) throw new Error('fixture-private SQL detail')
    },
  })
...
test('lost commit acknowledgement fails even when deletion committed', async () => {
  ...
  const f = cleanupFixture({
    ...
    afterCommit: () => { throw new Error('fixture-private commit response lost') },
  })
  await assert.rejects(cleanupAuthentication(f.source), { message: 'Authentication cleanup failed' })
  assert.equal(f.deleted.sessions.length, 1)
})
