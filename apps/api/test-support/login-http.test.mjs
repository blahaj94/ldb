...
test('HTTP rejects client-provided identity/redirect fields before service and accepts UTF-8 media', async () => {
  ...
  assert.equal((await post('/auth/login-requests', [atLimit.slice(0, 8000), atLimit.slice(8000)])).status, 201)
...
function rawHttp(wire) {
  ...
  return new Promise((resolve, reject) => {
    ...
    socket.setTimeout(3000, () => { socket.destroy(); reject(new Error('raw HTTP did not close')) })
    socket.once('connect', () => socket.write(wire))
    socket.on('data', (chunk) => { result += chunk.toString('utf8') })
...
test('stream overflow responds before body end; framing rejection is separate and sanitized', async () => {
  ...
  const overflow = await rawHttp('POST /auth/exchange HTTP/1.1\r\nHost: test.invalid\r\nContent-Type: application/json\r\nTransfer-Encoding: chunked\r\n\r\n' +
    '2000\r\n' + 'x'.repeat(8192) + '\r\n2001\r\n' + 'x'.repeat(8193) + '\r\n')
  ...
  const framing = await rawHttp('POST /auth/exchange HTTP/1.1\r\nHost: test.invalid\r\nContent-Type: application/json\r\nContent-Length: 1\r\nTransfer-Encoding: chunked\r\n\r\ncredential-canary')
...
test('auth application and error log sinks omit raw URL/body/cookie/identity and credentials', async () => {
  ...
  const result = await promisify(execFile)(process.execPath, ['--import', 'reflect-metadata', 'test-support/login-log-probe.mjs'], {
    cwd: new URL('..', import.meta.url), env: { PATH: process.env.PATH }, timeout: 5000,
  })
  assert.equal(result.stdout, 'Login log probe passed\n')
  assert.equal(result.stderr, '')
  const authorize = await fetch(`${base}/auth/login/authorize?ticket=${opaque()}`, { redirect: 'manual' })
