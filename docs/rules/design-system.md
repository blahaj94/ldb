...
SEED 채택 변경분은 [채택 결정](https://github.com/blahaj94/ldb/issues/86#issuecomment-5560112909)을 근거로 이 변경을 담은 Draft PR에서 제안한다. 기존 active Rule의 승인은 유지하며, 추가 Rule·architecture·dependency의 구현 권한은 해당 PR의 명시적인 승인과 작업의 실행 조건으로 확인한다. 별도 metadata 활성화 작업을 요구하지 않는다. 제품 behavior·API·인증·domain·platform contract는 변경하지 않는다.
