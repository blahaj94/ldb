...
const providerPkce = ['provider_pkce_ciphertext', 'provider_pkce_iv', 'provider_pkce_tag', 'provider_pkce_key_id']
...
const secretFields = ['code_challenge', 'method', 'launch_ticket_hash', ...browserContext, ...providerPkce, ...exchange]
const required = {
  created: ['code_challenge', 'method', 'launch_ticket_hash'],
  browser_started: ['code_challenge', 'method', 'state_hash', 'browser_binding_hash', 'oidc_nonce_hash', ...providerPkce],
  processing: ['code_challenge', 'method', 'state_hash', 'browser_binding_hash', 'oidc_nonce_hash', ...providerPkce],
...
function row(status, overrides = {}) {
  ...
  for (const field of ['launch_ticket_hash', 'state_hash', 'exchange_code_hash']) {
    if (request[field] !== null) request[field] = randomBytes(32)
  }
...
async function accept(dataSource, request) {
  ...
  try {
    ...
    const [{ status }] = await runner.query('SELECT status FROM auth_login_requests WHERE id = $1', [request.id])
...
export async function assertLoginRequestStateMatrix(dataSource) {
  ...
  for (const status of states) {
    ...
    for (const field of required[status]) {
      const constraint = providerPkce.includes(field) && status === 'processing' ? 'pkce_fields' : `${status}_fields`
    ...
    for (const field of forbidden[status]) {
      ...
      const values = {
        ...sample, ...Object.fromEntries(providerPkce.map((key) => [key, row('browser_started')[key]])),
        state_hash: randomBytes(32), browser_binding_hash: randomBytes(32), oidc_nonce_hash: randomBytes(32),
        launch_ticket_hash: randomBytes(32), consumed_at: new Date('2026-09-05T00:01:00Z'),
      }
  ...
  await accept(dataSource, row('exchange_ready', Object.fromEntries([...browserContext, ...providerPkce].map((key) => [key, retained[key]]))))
  accepted++

  for (const [field, suffix] of [
    ['launch_ticket_hash', 'launch_hash_length'], ['state_hash', 'state_hash_length'],
    ['browser_binding_hash', 'browser_hash_length'], ['oidc_nonce_hash', 'nonce_hash_length'],
    ['exchange_code_hash', 'exchange_hash_length'],
  ]) {
    for (const length of [31, 33]) {
      await rejectConstraint(dataSource, `ck_auth_login_requests_${suffix}`, (runner) =>
        insertLogin(runner, row(field === 'launch_ticket_hash' ? 'created' : 'exchange_ready', { [field]: Buffer.alloc(length) })))
      rejected++
    }
  }
  for (const [field, value] of [
    ['provider_pkce_ciphertext', Buffer.alloc(0)], ['provider_pkce_iv', Buffer.alloc(11)],
    ['provider_pkce_iv', Buffer.alloc(13)], ['provider_pkce_tag', Buffer.alloc(15)],
    ['provider_pkce_tag', Buffer.alloc(17)], ['provider_pkce_key_id', ''],
  ]) {
