...
async function assertDisconnected(source, runtime) {
  ...
  for (const pid of pids) await assertBackendGone(source, pid)
...
export async function assertRuntimeFreshStart(configuration, mark = () => {}) {
  await withDataSource(createDatabaseDataSource, configuration, async (source) => {
    ...
    await withRuntimeConfiguration(async ({ path }) => {
      ...
      const runtime = startRuntime(runtimeEnvironment(path, port, configuration), { realDatabase: true })
      try {
...
export async function assertRuntimeDatabaseFailures(configuration, mark = () => {}) {
  await withDataSource(createDatabaseDataSource, configuration, async (source) => {
    const before = await databaseSnapshot(source)
    for (const fault of ['partial-connect', 'app-create', 'nest-provider', 'app-configure', 'listen']) {
      mark(`${fault}: startup fails safely and its PostgreSQL backend disappears`)
      const server = createServer()
      const hasOccupiedPort = fault === 'listen'
      if (hasOccupiedPort) await new Promise((resolve) => server.listen(0, resolve))
      try {
        await withRuntimeConfiguration(async ({ path }) => {
          const port = hasOccupiedPort ? server.address().port : await unusedRuntimePort()
          const runtime = startRuntime(runtimeEnvironment(path, port, configuration), { realDatabase: true, fault })
          try {
        ...
        if (hasOccupiedPort) await new Promise((resolve) => server.close(resolve))
      }
    }
  })
}
...
async function accountRequest(client, accessToken, nickname) {
  ...
  return fetch(`${client.base}${isMutation ? '/me/nickname' : '/me'}`, {
    ...
    headers: { authorization: `Bearer ${accessToken}`, ...(isMutation ? { 'content-type': 'application/json' } : {}) },
...
async function assertSearchShutdown(source, runtime, client, tokens, principal, neople) {
  ...
  try {
    ...
    if (hasResponse) assert.equal(result.response.status, 500)
  ...
  try {
    await withDataSource(createDatabaseDataSource, configuration, async (source) => {
      ...
      await withRuntimeConfiguration(async ({ path, configuration: auth }) => {
        ...
        const options = { realDatabase: true, upstreams: { google: google.origin, neople: neople.origin } }
        ...
        const next = { ...previous, version: 'test-v2', returnTarget: { ...previous.returnTarget, id: 'test-return-v2' } }
        ...
        auth.google.secrets.push({
          version: next.version, reference: previous.providerSecretRef, value: 'fixture-different-active-secret',
        })
        ...
        mark('historical callback uses exact version/secret and the process reads configuration only once')
        ...
        assert.deepEqual(await changed.json(), { user: { id: tokens.user.id, nickname: '기본 실행 사용자' } })
        neople.upstream.body = { rows: [{ characterId: 'fixture-character', characterName: 'ab', serverId: 'cain', fame: 0 }] }
        const found = await search(client, tokens.accessToken)
        assert.equal(found.status, 200)
        assert.deepEqual(await found.json(), { rows: [{
          characterId: 'fixture-character', characterName: 'ab', serverId: 'cain', serverName: '카인', fame: 0,
        }] })
        assert.equal(neople.calls.length, 1)
        assert.equal(neople.calls[0].hasExpectedKey, true)

        mark('same app refreshes and logs out, denies both account routes but permits residual JWT search')
        ...
        assert.equal((await client.post('/auth/logout', { refreshToken: rotated.refreshToken })).status, 204)
        ...
        assert.equal((await client.post('/auth/refresh', { refreshToken: rotated.refreshToken })).status, 401)
        assert.equal(neople.calls.length, 2)
        const [session] = await source.query('SELECT revoked_reason FROM auth_sessions WHERE id=$1', [principal.sessionId])
        assert.equal(session.revoked_reason, 'logout')
        assert.deepEqual(await databaseSnapshot(source), beforeSchema)

        mark('SIGTERM cancels a real locked search before app and DB shutdown; upstream is not called')
    ...
    for (const runtime of runtimes) await stopRuntime(runtime)
    await google.close()
    await neople.close()
  }
}
