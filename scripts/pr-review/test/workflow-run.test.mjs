...
import { evaluateTrustedReviewRequest, pullRequestNumber } from '../src/workflow-run.mjs'
...
test('rejects failed source workflows', () => {
  ...
  assert.equal(evaluateTrustedReviewRequest(event, pullRequest).reason, 'source_workflow_failed')
})
...
test('rejects an outdated source head', () => {
  ...
  assert.equal(evaluateTrustedReviewRequest(event, pullRequest).reason, 'stale_head')
})
