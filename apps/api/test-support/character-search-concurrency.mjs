...
async function additionalSession(source, f, sameAccount) {
  const identity = sameAccount ? f.identity : { provider: 'google', subject: 'synthetic-other-search-account' }
  const initial = await source.transaction('READ COMMITTED', (manager) => createIdentitySession(manager, identity))
  const now = await databaseNow(source)
  await source.query('UPDATE auth_sessions SET created_at=$2,last_active_at=$2 WHERE id=$1', [
    initial.session.id, new Date(now.getTime() - 10_000),
  ])
  const token = await f.deps.issueAccessJwt({
    userId: initial.user.id, sessionId: initial.session.id,
    issuedAt: now.getTime() / 1000, idleDeadline: now.getTime() / 1000 + 900,
  })
...
async function concurrentQuota(source) {
  ...
  await withSearchApp(f, async ({ base, calls, upstream }) => {
    const responses = []
    upstream.respond = (_request, response) => { responses.push(response) }
    const requests = Array.from({ length: 11 }, (_, index) => searchRequest(
      base, index % 2 === 0 ? f : device, `characterName=ab&limit=${index + 1}`,
    ))
    try {
      ...
      await expectSearchError(await searchRequest(base, f, 'characterName=ab&limit[]=1'), 400, 'INVALID_SEARCH_QUERY')
      assert.equal(commits, 10)
      const independent = searchRequest(base, other)
      await waitFor(() => calls.length === 11)
      assert.equal(commits, 11)
      for (const response of responses) response.end('{"rows":[]}')
  ...
  assert((await snapshot(source, device)).session.last_active_at > beforeDevice.session.last_active_at)
}
...
async function finalReservationClock(source) {
  ...
  const clock = {
    now() {
      ...
      queueMicrotask(() => { yieldedAfterClock = true })
      return now
    },
  ...
  await withSearchApp(f, async ({ base, calls, upstream }) => {
    upstream.start = () => { assert.equal(yieldedAfterClock, false) }
...
export async function assertSearchConcurrency(source, mark) {
  const cases = [
    ['11 concurrent requests sum sessions and queries while another account and upstream responses remain independent', () => concurrentQuota(source)],
    ['post-commit final monotonic reservation and no microtask yield before adapter call', () => finalReservationClock(source)],
  ]
