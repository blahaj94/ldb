...
async function rejectedBeforeInitialization(environment) {
  ...
  try {
    ...
    assert.deepEqual(eventsOf(runtime), [], 'invalid configuration must not initialize DB, create an app or listen')
  } finally {
    await stopRuntime(runtime)
  }
}
  ...
  await withRuntimeConfiguration(async ({ path }) => {
    ...
    for (const name of Object.keys(valid)) {
      for (const value of [undefined, '']) {
        ...
        await t.test(`${name} ${kind}`, () => rejectedBeforeInitialization({ ...valid, [name]: value }))
      }
    }
    for (const value of ['0', '65536', '1.5', '+1', '-1', ' 1', '１']) {
      await t.test(`invalid PORT ${value}`, () => rejectedBeforeInitialization({ ...valid, PORT: value }))
    }
    for (const value of ['0', '65536', '1.5', 'port-canary']) {
      await t.test(`invalid DB_PORT ${value}`, () => rejectedBeforeInitialization({ ...valid, DB_PORT: value }))
    }
    for (const [name, value] of [
      ['relative path', 'auth.json'], ['unexpanded home', '~/auth.json'],
      ['missing file', `${path}.missing`], ['directory', path.slice(0, path.lastIndexOf('/'))],
    ]) {
...
test('build entry rejects malformed authentication JSON and exact binding violations before DB', async (t) => {
  await withRuntimeConfiguration(async ({ path, configuration }) => {
    ...
    const cases = [
      ['missing root field', (c) => { delete c.accessJwt }],
      ['unknown root field', (c) => { c.testMode = true }],
      ['wrong issuer type', (c) => { c.accessJwt.issuer = 12 }],
      ['unknown signing field', (c) => { c.accessJwt.signingKey.extra = true }],
      ['missing signing PEM', (c) => { delete c.accessJwt.signingKey.privateKeyPem }],
      ['unusable signing PEM', (c) => { c.accessJwt.signingKey.privateKeyPem = 'private-key-canary' }],
      ['unregistered signing kid', (c) => { c.accessJwt.signingKey.kid = 'unknown-key' }],
      ['duplicate verification kid', (c) => { c.accessJwt.verificationKeys.push(c.accessJwt.verificationKeys[0]) }],
      ['wrong verification collection', (c) => { c.accessJwt.verificationKeys = {} }],
      ['wrong verification element', (c) => { c.accessJwt.verificationKeys = [null] }],
      ['missing active PKCE key', (c) => { c.providerPkce.activeKeyId = 'unknown-key' }],
      ['duplicate PKCE key id', (c) => { c.providerPkce.keys.push(c.providerPkce.keys[0]) }],
      ['padded PKCE encoding', (c) => { c.providerPkce.keys[0].key += '=' }],
      ['noncanonical PKCE encoding', (c) => { c.providerPkce.keys[0].key = `${'A'.repeat(42)}B` }],
      ['wrong PKCE length', (c) => { c.providerPkce.keys[0].key = 'A'.repeat(42) }],
      ['wrong PKCE value type', (c) => { c.providerPkce.keys[0].key = [] }],
      ['unknown PKCE field', (c) => { c.providerPkce.keys[0].extra = true }],
      ['missing registry snapshot', (c) => { c.registry.registrations = [] }],
      ['unregistered active version', (c) => { c.registry.activeVersions.google = 'unknown-version' }],
      ['unsupported active provider', (c) => { c.registry.activeVersions.discord = 'test-v1' }],
      ['unsupported registration', (c) => { c.registry.registrations[0].provider = 'discord' }],
      ['duplicate registry version', (c) => { c.registry.registrations.push(c.registry.registrations[0]) }],
      ['wrong audience binding', (c) => { c.registry.registrations[0].expectedAudience = 'wrong-client' }],
      ['wrong callback binding', (c) => { c.registry.registrations[0].callbackUrl = 'https://other.invalid/auth/callback/google' }],
      ['unknown return target field', (c) => { c.registry.registrations[0].returnTarget.extra = true }],
      ['missing Google endpoint', (c) => { c.google.registrations = [] }],
      ['duplicate Google endpoint', (c) => { c.google.registrations.push(c.google.registrations[0]) }],
      ['orphan Google endpoint', (c) => { c.google.registrations[0].version = 'unknown-version' }],
      ['HTTP token endpoint', (c) => { c.google.registrations[0].tokenEndpoint = 'http://not-trusted.invalid/token' }],
      ['JWKS userinfo', (c) => { c.google.registrations[0].jwksUri = 'https://user:password@keys.invalid/certs' }],
      ['unknown Google transport', (c) => { c.google.fetch = 'test-override' }],
      ['missing secret', (c) => { c.google.secrets = [] }],
      ['duplicate secret tuple', (c) => { c.google.secrets.push(c.google.secrets[0]) }],
      ['wrong secret version', (c) => { c.google.secrets[0].version = 'unknown-version' }],
      ['wrong secret reference', (c) => { c.google.secrets[0].reference = 'unknown-reference' }],
      ['blank secret', (c) => { c.google.secrets[0].value = '  ' }],
      ['wrong secret type', (c) => { c.google.secrets[0].value = null }],
      ['unknown secret field', (c) => { c.google.secrets[0].path = '/unused-secret' }],
    ]
...
test('build entry connects once and closes the app before its DB on termination', async (t) => {
  for (const signal of ['SIGINT', 'SIGTERM']) {
    await t.test(signal, () => withRuntimeConfiguration(async ({ path }) => {
      const port = await unusedRuntimePort()
      const runtime = startRuntime(runtimeEnvironment(path, port))
      try {
        await waitForRuntime(port, runtime)
        ...
        assert.deepEqual(eventsOf(runtime), [
          'db.initialize', 'app.create', 'app.listen', 'app.close', 'app.closed', 'db.destroy', 'db.disconnected',
        ])
      } finally {
        await stopRuntime(runtime)
      }
...
test('build entry releases owned resources after partial setup and app cleanup failures', async (t) => {
  const cases = [
    ...
    ['app-configure', ['db.initialize', 'app.create', 'app.close', 'app.closed', 'db.destroy', 'db.disconnected']],
    ['app-close', ['db.initialize', 'app.create', 'app.listen', 'app.close', 'app.closed', 'db.destroy', 'db.disconnected']],
  ]
