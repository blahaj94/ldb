...
export function authenticationConfiguration() {
  ...
  return {
    accessJwt: {
      issuer: 'https://issuer.test.invalid', audience: 'runtime-test-api',
      signingKey: {
        kid: 'runtime-test-key', privateKeyPem: keys.privateKey.export({ type: 'pkcs8', format: 'pem' }),
      },
      verificationKeys: [{
        kid: 'runtime-test-key', publicKeyPem: keys.publicKey.export({ type: 'spki', format: 'pem' }),
      }],
    },
    providerPkce: {
    ...
    registry: {
      ...
      activeVersions: { google: snapshot.version }, registrations: [snapshot],
    },
    google: {
      registrations: [{ version: snapshot.version, tokenEndpoint, jwksUri }],
      secrets: [{ version: snapshot.version, reference: snapshot.providerSecretRef, value: 'fixture-client-secret' }],
    },
  }
}

export async function withRuntimeConfiguration(operation, configuration = authenticationConfiguration()) {
...
export function databaseEnvironment(configuration = {
  host: '127.0.0.1', port: 1, username: 'runtime-test', password: 'runtime-test-password', database: 'runtime-test',
}) {
  return {
    DB_HOST: configuration.host, DB_PORT: String(configuration.port), DB_USERNAME: configuration.username,
    DB_PASSWORD: configuration.password, DB_NAME: configuration.database,
  }
}

export function startRuntime(environment, { fault = '', realDatabase = false, upstreams = {} } = {}) {
  ...
  const child = spawn(process.execPath, [
    '--import', 'reflect-metadata', '--import', './test-support/runtime-preload.mjs', 'dist/main.js',
...
export async function stopRuntime(runtime) {
  ...
  if (isRunning) runtime.child.kill('SIGKILL')
...
export function runtimeEnvironment(path, port, database) {
  return {
    ...databaseEnvironment(database), AUTH_CONFIG_FILE: path, PORT: String(port),
