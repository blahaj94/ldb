...
이 절은 [사용자 merge 승인](change-control.md#approval-evidence)을 위해 검토하는 추천안이다. 상세 상황표·추가 rotation 대안·후속 Red→Green 검증 계획은 Issue/PR에 둔다. 이 절의 substantive contract와 status 전환을 채택 범위로 명시한 PR을 사용자가 merge할 때만 승인·활성화되며, 절차 문구나 link만 수정하면 추천안 상태를 유지한다. 제품 반영은 그 merge와 별도의 사용자 실행 범위에서 진행한다.
