...
test('Schema First generation writes executable ESM-safe SQL and reverses rollback order without applying it', async () => {
  ...
  try {
    ...
    await assert.rejects(
      new Migration().up({ ...runner, isTransactionActive: false }),
      /active transaction/
    )
  } finally {
...
test('generation rejects unsafe names, reports no changes and sanitizes connection failures', async () => {
  ...
  const source = {
    ...
    driver: {
      createSchemaBuilder: () => ({ log: async () => ({ upQueries: [], downQueries: [] }) })
    },
    destroy: async () => {
      source.isInitialized = false
    }
  ...
  try {
    assert.equal(
      await generateMigration('NoChanges', () => source as unknown as DataSource, directory),
      'Database schema is current'
    )
    assert.deepEqual(await readdir(directory), [])
    await assert.rejects(
      generateMigration('../Escape', () => source as unknown as DataSource, directory)
    )
    await assert.rejects(
      generateMigration(
        'Failure',
