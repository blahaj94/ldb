...
import { searchFixture, withSearchApp, searchRequest, observeSearchRunners } from './character-search-fixtures.mjs'
...
async function activityFirst(source, cleanup) {
  ...
  const createQueryRunner = observeSearchRunners({
    commit: async (runner, commit) => {
      activityAt = (await runner.query('SELECT last_active_at FROM auth_sessions WHERE id=$1', [f.initial.session.id]))[0].last_active_at
  ...
  await withSearchApp(f, async ({ base, calls }) => {
    ...
    try {
      ...
      await cleanupWaitingOn(source, cleanup, 'auth_sessions', f.initial.session.id, pid, () => release.resolve())
...
async function cleanupFirst(source, cleanup) {
  ...
  const createQueryRunner = observeSearchRunners({
    query: async ({ sql, parameters, query, run }) => {
      const isLock = targets(sql, parameters, 'SELECT', 'auth_sessions', f.initial.session.id) && sql.includes('FOR UPDATE')
      if (isLock) activityLock.resolve((await query('SELECT pg_backend_pid() AS pid'))[0].pid)
      return run()
    },
  })
  let signingCalls = 0
  ...
  await withSearchApp(f, async ({ base, calls }) => {
    await withCleanupDeletionHeld(source, cleanup, 'auth_sessions', f.initial.session.id, async ({ pid, waiter, release }) => {
  ...
  assert.deepEqual(await source.query('SELECT * FROM users WHERE id=$1', [f.initial.user.id]), userBefore)
}
...
async function staleSessionHint(source, cleanup, change) {
  ...
  await withLock(source, 'auth_sessions', f, async ({ runner, pid }) => {
    await cleanupWaitingOn(source, cleanup, 'auth_sessions', f.initial.session.id, pid, async () => {
      const canCommit = runner.isTransactionActive
      if (!canCommit) return
      const shouldDelete = change === 'deleted'
      if (shouldDelete) await runner.query('DELETE FROM auth_sessions WHERE id=$1', [f.initial.session.id])
      else await runner.query('UPDATE auth_sessions SET user_id=$2 WHERE id=$1', [f.initial.session.id, other.initial.user.id])
      await runner.commitTransaction()
    })
  })
  const final = await stored(source, f.initial.session.id)
  const wasDeleted = change === 'deleted'
  if (wasDeleted) assert.deepEqual(final, { session: undefined, tokens: [] })
  else {
...
export async function assertCleanupSessionConcurrency(source, cleanup, mark) {
  const cases = [
    ...
    ['cleanup holds deletion while refresh and residual search wait', () => cleanupFirst(source, cleanup)],
    ['session disappears after cleanup hint', () => staleSessionHint(source, cleanup, 'deleted')],
    ['session ownership changes after cleanup hint', () => staleSessionHint(source, cleanup, 'ownership')],
  ]
