...
export async function runAuthenticationCleanup(createDataSource: () => DataSource): Promise<CleanupResult> {
  ...
  if (hasSource) {
    try {
      ...
      if (ownedSource.isInitialized) await ownedSource.destroy()
      else await ownedSource.driver.disconnect()
    } catch {
      failed = true
    }
  }
  ...
  if (cannotConfirmSuccess) throw cleanupFailure()
