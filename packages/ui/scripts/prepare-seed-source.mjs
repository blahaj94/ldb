...
for (const file of provenance.files) {
  ...
  if (!response.ok) {
    throw new Error(`Pinned SEED source fetch failed: ${response.status}`)
  }
  ...
  if (!hasExpectedSource) {
    throw new Error(`Pinned SEED source checksum mismatch: ${file.upstream}`)
  }
  ...
  if (isDialog) {
    ...
    if (!hasExpectedExport) {
      throw new Error('Pinned DialogTrigger export changed')
    }
    localSource = upstream.replace(triggerExport, portableTriggerExport)
    file.localChanges.push({
      reason:
        'Declaration이 pnpm의 private transitive 경로를 참조하지 않도록 공식 export type을 명시한다. Runtime 변경 없음.',
  ...
  if (isLayout) {
    ...
    for (const change of changes) {
      ...
      if (!hasExpectedContent) {
        throw new Error('Pinned Layout content changed')
      }
  ...
  if (isModifiedSource) {
    ...
    if (isExportMissing) {
      throw new Error('Modified SEED source export is missing')
    }
