...
function fixture(t) {
  ...
  const calls = [];
  const issue = { number: 30, title: "Test task", url: "https://example.invalid/issues/30", state: "OPEN" };
  const run = (command, args, options) => {
    calls.push([command, ...args]);
    if (command === "gh") return JSON.stringify(issue);
    return execFileSync(command, args, {
      ...options, cwd: repository, stdio: ["ignore", "pipe", "pipe"],
    });
  };
...
test("rejects invalid arguments without external commands", () => {
  for (const args of [[], ["30"], ["0", "target"], ["-1", "target"], ["1;echo", "target"], ["9007199254740992", "target"], ["30", " "], ["30", "target", "extra"]]) {
...
test("closed or mismatched Issue and GitHub failure do not fetch or create a worktree", (t) => {
  ...
  assert.throws(() => startTask(["30", f.destination], () => { throw new Error("GitHub unavailable"); }), /GitHub unavailable/);
...
test("CLI reports invalid input with a failing exit status", () => {
  const result = spawnSync(process.execPath, [fileURLToPath(new URL("../start-task.mjs", import.meta.url))], { encoding: "utf8" });
