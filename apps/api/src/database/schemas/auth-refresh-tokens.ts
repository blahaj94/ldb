...
export const AuthRefreshTokenSchema = new EntitySchema<AuthRefreshToken>({
  ...
  foreignKeys: [
    {
      name: 'fk_auth_refresh_tokens_session',
      target: 'AuthSession',
  ...
  indices: [
    {
      name: 'uq_auth_refresh_tokens_unconsumed_session',
      columns: ['sessionId'],
      unique: true,
  ...
  checks: [
    ...
    {
      name: 'ck_auth_refresh_tokens_consumed_time',
      expression: `"consumed_at" IS NULL OR "consumed_at" >= "issued_at"`
    }
  ]
})
