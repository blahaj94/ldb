...
`some`, `find`, `filter`라는 이름만으로 callback 전체를 면제하지 않는다. 연산과 입력의 역할, callback의 단일 선택·일치 의미가 짧고 응집된 표현만으로 분명할 때만 직접 표현을 고려한다. 단순한 글자 수나 비교 수는 근거가 아니다. 순회나 table을 배열 index별 hardcoding으로 바꾸지 않는다.

두 예외 모두 반환값, 오류·schema·보안 정책, 논리의 극성, 필요한 type·존재 보호를 유지한다. 단순함은 순수성을 증명하지 않는다. TypeScript `readonly`만으로 getter나 Proxy가 없다고 판단하지 않는다. getter, Proxy, 사용자 정의 coercion, 시계·난수·I/O·상태 변경·async, 실패와 예외 우선순위가 있거나 불확실하면 기존 기준을 적용한다.

## 예외 A: 응집된 collection 선택

collection operation과 단일 predicate가 하나의 선택·일치 의미를 직접 드러내면 boolean을 callback 내부의 여러 이름으로 나누지 않을 수 있다. 허용 예시는 다음과 같다.
...
이 예시는 `RULE_PATHS`를 순회하고 각 path가 directory prefix 또는 exact filename인지 선택한다는 의도가 표현 자체에 드러난다. 복합 validation, domain 권한, type guard, 부작용, 여러 처리 단계가 callback에 들어가면 기본 명명·합성 기준을 유지한다. callback에서 외부 상태를 읽거나 바꾸거나 예외를 분류하면 이 예외를 사용하지 않는다.

## 예외 B: 독립 pure check의 평가 분리

기존 short-circuit 앞 guard가 뒤 검사를 보호하지 않고, 두 검사가 입력 type·값이 확정된 뒤 수행되는 독립 pure check라면 각 boolean을 따로 계산하고 최종 boolean으로 합성할 수 있다. 예시는 다음과 같다.
...
이 예외의 근거는 각 정규식이 현재 `i` flag만 사용하고 `g`·`y`가 없으며, `filename`이 문자열이고 두 검사가 pure하다는 확인이다. 원래 앞 검사가 false여서 뒤 검사가 실행되지 않던 입력에서도 뒤 검사를 실행해도 안전하고, 결과·오류·관찰 가능한 상태·업무 비용 제한이 변하지 않는다는 근거가 모두 있어야 한다. 평가 횟수 완화는 이처럼 독립 pure check에만 허용한다.
...
코드 예시는 설명용이며 제품 선택이나 새 정책을 정의하지 않는다. 변경에서 평가 횟수, getter·Proxy 관찰, 오류 우선순위 또는 상태 결과가 달라질 가능성이 있으면 해당 범위를 보류하고 기존 change-control·testing 절차와 사람 판단을 따른다.
