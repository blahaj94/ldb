...
export class CharacterSearchController {
  constructor(@Inject(CHARACTER_SEARCH_SERVICE) private readonly service: AuthenticatedSearchHttpService) {}

  @Get()
  async search(@Req() request: Request, @Res() response: Response): Promise<void> {
    const isGet = request.method === 'GET'
    if (!isGet) throw neopleSearchFailure('query')
    ...
    try {
      const result = await this.service.search(request.rawHeaders, request.originalUrl, controller.signal)
      const canRespond = !response.destroyed
      if (canRespond) response.status(200).json(result)
    } finally {
