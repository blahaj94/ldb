...
test('AuthLoginRequest refactor preserves flat columns, defaults and exact named CHECK SQL', async () => {
  const expected: EntitySchemaOptions<AuthLoginRequest> = JSON.parse(
    await readFile(
      new URL('../../test/fixtures/auth-login-request-schema.json', import.meta.url),
      'utf8'
    )
  )
  const actual = AuthLoginRequestSchema.options
  const sortChecks = (checks: typeof actual.checks) =>
    [...(checks ?? [])].sort((a, b) => (a.name ?? '').localeCompare(b.name ?? ''))
  assert.deepEqual(Object.keys(actual.columns), Object.keys(expected.columns))
  assert.deepEqual(
    { ...actual, checks: sortChecks(actual.checks) },
    { ...expected, checks: sortChecks(expected.checks) }
  )
})
