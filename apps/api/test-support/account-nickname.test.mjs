...
test('nickname uses the runtime extended grapheme contract and preserves Unicode', async (t) => {
  ...
  t.diagnostic(`Node ${process.version}; Unicode ${process.versions.unicode}; ICU ${process.versions.icu}`)
  const allowed = [
    ['  모험가  ', '모험가'],
    ['\u00a0e\u0301\u00a0', 'e\u0301'],
    ['👩🏽‍🚀'.repeat(20), '👩🏽‍🚀'.repeat(20)],
  ...
  for (const [input, expected] of allowed) assert.equal(validateNickname(input), expected)
})
...
test('nickname rejects raw controls before trim, malformed UTF-16, empty and 21 graphemes', async () => {
  ...
  const invalid = [
    ...
    '\ud800', '\udc00', 'a\ud800b', '\udc00\ud800',
    '\ta', 'a\n', '\ra', '\u0000a', '\u0085a', '\u009fa',
    '\u2028a', 'a\u2029', 'a'.repeat(21), '👩🏽‍🚀'.repeat(21),
  ]
  for (const input of invalid) {
    assert.throws(() => validateNickname(input), (error) => {
