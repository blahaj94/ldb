...
# Character Search Contract Proposal

이 문서는 승인 전 결정안이다. 아래 **논의 확정**은 RFC의 논의 상태이며 승인된 repository Rule을 뜻하지 않는다. **추가 제안**과 함께 [`change-control.md`](change-control.md)의 Draft PR `승인`을 받은 범위만 후속 Execution Issue에서 구현한다. 검색 결과 저장·캐싱, OCR 보정, 인증·DB 구현과 전체 서비스 한도는 이 안의 범위 밖이다.
...
1. Raw query를 `&`로 나누고 각 component의 첫 `=`만 key/value 경계로 사용한다. `=` 없는 key는 empty value다. 비어 있는 component(`&&`, 마지막 `&`)도 잘못된 입력으로 제안한다.
2. Key와 value에서 `+`를 space로 처리하고 percent-encoded UTF-8을 정확히 한 번 decode한다. 잘못된 `%` escape나 유효하지 않은 UTF-8은 400으로 제안한다. `URLSearchParams`의 관대한 오류 복구 결과만 사용하지 않는다.
...
4. Decode한 값에 확정 규칙을 적용한다. `characterName !== characterName.trim()`이면 400이며 공백 판별은 ECMAScript `trim`을 제안한다. 내부 공백은 별도 금지하지 않는다. `serverId`도 trim·case 변환 없이 map 또는 `all`과 비교한다.
5. `limit`는 ASCII `/^[0-9]+$/`와 숫자 범위 1~200을 모두 만족해야 한다. Leading zero는 십진 표기로 허용하는 안이다(`0010` → 10). 부호·소수점·지수·앞뒤 공백·비ASCII 숫자는 400이다. `parseInt`의 부분 성공은 사용하지 않는다.
...
**제안은 decode 후 Unicode code point 수 2~12**다. JavaScript string iterator로 세며 정규화하지 않는다. `가나`는 2, `😀`는 1, `😀가`는 2, 분해된 `가`는 2다. 공백 검증은 길이와 별도로 적용한다.
...
| 선택 | 영향 |
| --- | --- |
| Code point 제안 | Supplementary character를 하나로 세며 결합 sequence는 여러 개. 별도 dependency 없음. |
| UTF-16 code unit | JavaScript `length`와 같지만 supplementary character 하나가 둘로 셈. |
| Grapheme | 화면상의 글자에 가까우나 Unicode segmentation과 외부 검색 제한이 같다고 보장할 수 없음. |

사용자가 우리 입력 정책을 승인하고 외부 단위 미확인 위험을 수용하거나, 별도로 외부 규격을 확인한 뒤 다른 안을 승인해야 query 연결을 시작한다. 우리 검증을 통과한 query를 네오플이 거절하는 경우 아래 502 mapping을 유지하고 임의로 길이·정규화 정책을 바꾸지 않는다. 이 결정은 credential 없는 runtime·adapter 검증을 막지 않는다.
...
## 논의 확정과 추가 제안: Deadline과 adapter

네오플 호출 대기는 최대 5초, timeout은 504, 서버 자동 retry는 0회다. 추가 제안은 transport 호출 직전부터 headers와 전체 body 수신·검증 완료까지 하나의 5,000ms deadline을 적용하는 것이다. 완료 판정 시각이 deadline **이상**이면 timeout이다. Headers만 빨리 도착해도 body가 늦으면 성공하지 않는다.
...
**논의 확정 수치**는 계정당 최근 60초 최대 10회, 기기·검색 조건 합산, 초과 시 upstream 없이 429와 `Retry-After`다. 아래 차감·시간·동시성·저장 범위는 새 정책 제안이며 사용자 승인이 필요하다.
...
**저장 범위 제안:** 단일 Node process의 memory에 최근 예약만 유지하고 만료 entry를 정리한다. 여러 기기는 동일 process의 account key로 합산한다. 재시작 시 제한 이력이 사라져 60초 안에 추가 호출이 허용될 수 있고, 여러 process/replica에서는 10회 보장이 성립하지 않는다. 이 제한을 수용하는 사용자 승인 전에는 memory 방식도 구현하지 않는다. 대안은 공유 저장소의 원자 예약이지만 새 dependency·운영 state/DB 계약이 필요하므로 별도 설계 대상이다. 검색 결과를 저장하는 안은 아니다.
...
아래는 새 제안이 승인되었을 때의 기대값이다. Upstream 횟수는 유효한 인증·설정과 여유 quota를 전제로 한다.
...
상세 acceptance matrix와 실행 evidence는 해당 Execution Issue/PR에 둔다. Runtime 도구와 실제 실행 계획은 [`api-runtime.md`](api-runtime.md), Red→Green 순서는 [`testing.md`](testing.md)를 따른다. 승인하지 않은 길이·raw decoding·deadline 세부안·quota 정책을 후속 구현자의 일반 선택으로 숨기지 않는다.
