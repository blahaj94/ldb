...
async function disconnectedRequest(source, queued) {
  ...
  const verifyAccessJwt = async (...args) => {
    ...
    if (isSecond) verifiedAgain.resolve()
    return principal
  }
  await withLock(source, 'auth_sessions', f, async ({ pid: blocker, unlock }) => {
    await withSearchApp(f, async ({ base, calls }) => {
      const controller = new AbortController()
      const first = settled(searchRequest(base, f, undefined, queued ? {} : { signal: controller.signal }))
      ...
      if (!queued) await assertBackendGone(source, pid)
      assert.equal(calls.length, 0)
      ...
      for (let count = priorCalls; count < 10; count += 1) assert.equal((await searchRequest(base, f)).status, 200)
...
async function shutdownDuringLock(source) {
  ...
  await withLock(source, 'auth_sessions', f, async ({ pid: blocker }) => {
    await withSearchApp(f, async ({ base, calls, app }) => {
      const pending = searchRequest(base, f)
      const pid = await bounded(locking.promise)
      await blockedBy(source, pid, blocker)
...
export async function assertSearchCancellation(source, mark) {
  const cases = [
    ...
    ['active HTTP disconnect aborts actual DB waiter without reservation', () => disconnectedRequest(source, false)],
    ['queued HTTP disconnect removes admission reference without opening a DB connection', () => disconnectedRequest(source, true)],
    ['application shutdown aborts and releases an actual DB lock waiter', () => shutdownDuringLock(source)],
    ['exhausted TypeORM pool does not create an uncancellable search acquisition waiter', () => exhaustedPool(source)],
  ]
