...
| 전체 현황판 | 현재 제품 계획, 여러 목표별 Project의 진입점과 작업 이력. GitHub Project로 구현할 수 있지만 단일 목표의 완료 단위가 아니다. |
| 목표별 Project | 완료 가능한 목표 하나의 현재 범위, 완료 조건, 내부 작업과 전체 진행 상황 |
| 목표별 Project 내부 작업 | 사용자가 선택할 중간 결과, 완료 조건, 선행 작업과 연결된 실행 Issue |
| Execution Issue | 구현과 검증이 가능한 단위의 현재 실행 계약, 승인, 담당과 검증 근거 |
| PR | 해당 Issue의 실제 변경, 완료 조건별 검증 결과와 사용자 merge |

이 문서에서 별도 수식 없이 사용하는 Project는 목표별 Project를 뜻한다. 전체 현황판에는 단일 목표의 범위와 완료 조건, 아래의 목표 완료에 따른 `Completed` 기록과 Close project 절차를 적용하지 않는다. 현황판의 역할 변경과 종료는 [기존 기록의 전환](#기존-기록의-전환)에 따라 사용자가 선택한다.
...
원본의 각 남은 결과마다 목적지 Project와 내부 작업, 또는 Discussion에 남기는 구체적인 이유를 대응표에 기록한다. 실행 작업만 옮기고 제품 목표나 미정 조건을 누락하지 않는다. 승인된 계획을 단순 아이디어로 분류하지 않으며, MVP 밖의 후속 구상은 그 논의 위치와 현재 제외 범위를 유지한다.

대응표의 단일 원본은 이관 담당 Issue에서 지정한 댓글 하나다. 담당 Issue 본문에 그 댓글의 링크를 명시하고, Project, Discussion과 PR에서는 같은 원본을 링크한다. 목적지나 이관 상태가 바뀌면 원본 댓글을 갱신하며 다른 위치에 현재 대응표를 복제하지 않는다. 별도 검증 결과는 당시 확인한 결과임을 표시하고 현재 대응표를 링크한다. 영구 Rule에는 backlog를 복제하지 않는다.
