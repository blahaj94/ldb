...
import { searchFixture, withSearchApp, searchRequest, expectSearchError, snapshot, observeSearchRunners, barrier, waitFor, assertBackendGone } from './character-search-fixtures.mjs'
...
async function exactTime(source, kind, offset) {
  ...
  if (isIdle) {
    ...
    f.token = await f.deps.issueAccessJwt({
      userId: f.initial.user.id, sessionId: f.initial.session.id, issuedAt, idleDeadline: issuedAt + 900,
    })
  ...
  let clocks = 0
  const createQueryRunner = observeSearchRunners({ query: async ({ sql, run }) => {
...
async function nearSecondBoundary(source) {
  ...
  await waitFor(async () => {
    const [{ fraction }] = await source.query('SELECT extract(epoch from clock_timestamp()) % 1 AS fraction')
...
async function realLockExpiry(source, kind) {
  ...
  if (isJwt) {
    f.token = await f.deps.issueAccessJwt({
      ...
      issuedAt: f.now.getTime() / 1000, idleDeadline: target.getTime() / 1000,
    })
  } else {
  ...
  const createQueryRunner = observeSearchRunners({ query: async ({ sql, query, run }) => {
    const isSessionLock = sql.includes('auth_sessions') && sql.includes('FOR UPDATE')
    if (isSessionLock) locking.resolve((await query('SELECT pg_backend_pid() AS pid'))[0].pid)
    return run()
  } })
...
async function residual(source, kind) {
  ...
  await withSearchApp(f, async ({ base, calls }) => {
    ...
    if (isExpiredRevoked) await setDeadline(source, f.initial.session.id, f.now)
    if (isDeletedUser) await source.query('DELETE FROM users WHERE id=$1', [f.initial.user.id])
    if (isDeletedSession) await source.query('DELETE FROM auth_sessions WHERE id=$1', [f.initial.session.id])
    const before = await snapshot(source, f)
...
async function removalAfterActivity(source, kind) {
  ...
  const before = await snapshot(source, f)
  let base
  const createQueryRunner = observeSearchRunners({ commit: async (_runner, commit) => {
    await commit()
    const isLogout = kind === 'logout'
    if (isLogout) {
      const response = await fetch(`${base}/auth/logout`, {
  ...
  await withSearchApp(f, async (context) => {
    base = context.base
    assert.equal((await searchRequest(base, f)).status, 200)
...
export async function assertSearchSessionBoundaries(source, mark) {
  const cases = [
    ...['jwt', 'idle', 'iat'].flatMap((kind) => [-1, 0].map((offset) => [
      `${kind} exact fresh DB boundary offset ${offset}`, () => exactTime(source, kind, offset),
    ])),
    ...['jwt', 'idle'].map((kind) => [`real session lock crosses ${kind} boundary`, () => realLockExpiry(source, kind)]),
    ...['logout', 'deleted-user', 'deleted-session', 'expired-revoked'].map((kind) => [
      `${kind} residual request preserves all remaining state`, () => residual(source, kind),
    ]),
    ...['logout', 'delete'].map((kind) => [`${kind} after activity commit preserves admitted search`, () => removalAfterActivity(source, kind)]),
    ['JWT expiry after admitted commit does not revoke request but rejects new admission', () => expiredAfterAdmission(source)],
  ]
