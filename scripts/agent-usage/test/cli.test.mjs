...
async function setup(t) {
  ...
  for (const [turn, timestamp] of [['first', time], ['second', nextTime]]) {
    events.push(event('turn_context', { turn_id: turn, model: 'gpt-test', effort: 'low' }, timestamp));
    events.push(event('token_usage_record', {
      thread_id: 'root', turn_id: turn, root_turn_id: turn, response_id: turn,
      usage: { input_tokens: 10, cached_input_tokens: 8, output_tokens: 2,
        reasoning_output_tokens: 1, total_tokens: 12 }
    }, timestamp));
  }
  await writeFile(join(sessions, 'root.jsonl'), events.map((row) => JSON.stringify(row)).join('\n') + '\n');
  const output = [];
  const options = {
    cwd: directory, env: { CODEX_HOME: join(directory, 'codex'), CODEX_THREAD_ID: 'root' },
    write: (text) => output.push(text),
    call: (args) => {
      if (args[0] === 'repo') return { nameWithOwner: 'owner/repo' };
      if (args[0] === 'pr') return {
        number: 3, url: 'https://github.com/owner/repo/pull/3', headRefOid: head,
        state: 'OPEN', mergedAt: null, isCrossRepository: false, author: { login: 'owner' },
        closingIssuesReferences: [{ number: 1, repository: { name: 'repo', owner: { login: 'owner' } } }]
      };
...
test('begin은 local manifest를 저장하고 시작 turn의 조용한 덮어쓰기를 거부한다', async (t) => {
  ...
  await runUsage(['begin', '--issue', '1', '--from-turn', 'first'], options);
  const manifest = JSON.parse(await readFile(join(directory, '.git/agent-usage/issue-1.json'), 'utf8'));
...
test('snapshot은 명시적 종료 범위로 요약하고 --publish 없이는 GitHub에 쓰지 않는다', async (t) => {
  ...
  await runUsage(['begin', '--issue', '1', '--from-turn', 'first'], options);
  await runUsage(['snapshot', '--issue', '1', '--pr', '3', '--through-turn', 'first', '--until', end, '--json'], options);
...
test('PR의 linked Issue가 다르면 snapshot을 저장하지 않는다', async (t) => {
  ...
  await assert.rejects(runUsage(['snapshot', '--issue', '2', '--pr', '3', '--until', end], options), /Issue/);
});

test('누락된 manifest 또는 잘못된 command/숫자 입력을 거부한다', async (t) => {
  const { options } = await setup(t);
  for (const args of [['begin', '--issue', '0'], ['begin', '--issue', '../1'],
    ['snapshot', '--issue', '1', '--pr', '3'], ['surprise']]) {
