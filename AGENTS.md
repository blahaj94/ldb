...
- 이 프로젝트의 모든 작업에서 서브 에이전트를 생성하거나 기존 서브 에이전트에 작업을 위임·재개하지 않는다. 구현뿐 아니라 조사, 계획, 검증, review와 명령 실행에도 적용한다.
- 다른 task, CLI 또는 도구를 통해 별도 AI agent를 실행하는 방식으로 이 금지를 우회하지 않는다.
- 현재 담당 에이전트가 작업을 직접 순차 수행한다. Planner, Worker, Reviewer, Runner와 통합 담당은 별도 에이전트를 만드는 근거가 아니며, 필요한 역할은 현재 담당 에이전트가 맡는다.
- 이 절은 하위 AGENTS.md와 다른 프로젝트 문서의 위임·병렬 agent 수행 지침 및 직접 수행 제한 조건보다 우선한다. Issue, 전용 branch·worktree, 승인, 검증, 보안과 사용자 merge 의무는 유지한다. 독립 review가 필요한 경우 사람에게 요청하며 자신의 검토를 독립 review로 보고하지 않는다.
