...
- URI scheme 형태가 전혀 없으면 일반 실행으로 분류해 activation만 호출하고, exact valid 후보 하나면 auth dispatch만 호출한다. Trusted scheme 후보가 malformed·여러 개이거나 앞쪽 whitespace/control·parser가 무시하는 내부 TAB/LF/CR을 포함한 다른 URL-like 입력이 있으면 둘 다 호출하지 않는다. Well-formed scheme뿐 아니라 path/query/fragment 구분자 없는 prefix 뒤의 colon과 slash 또는 backslash로 시작하는 hierarchical 형태도 malformed URL-like 입력군으로 취급한다. `--name=value`, `--name:value`, `/name=value`, `/name:value` 형태의 structured option은 payload에도 같은 URL-like 검사를 적용한다. 예외는 대소문자를 정규화한 이름이 정확히 `user-data-dir`이고 payload가 drive letter와 colon 뒤에 slash 또는 backslash가 정확히 하나인 absolute Windows drive 형태(`C:/...`, `C:\...`)일 때뿐이며, 이 값은 일반 argument다. Scheme 형태가 아닌 `/tmp/report:2026.txt`, `--label=12:30`, 내부 space 뒤 colon도 일반 argument다. 반대로 bootstrap에서 제거되지 않은 direct `C:\...`, `C:/...`, `C:relative` user argument와 `return-url`·`other-path` 같은 다른 option의 drive-shaped payload는 one-letter URI와 어휘적으로 모호하므로 fail closed한다. 이 분류는 detection projection으로 raw 값을 보정하지 않으며 세 결과는 단일 Electron listener가 소유한다.
