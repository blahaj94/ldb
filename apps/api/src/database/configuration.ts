...
function required(value: string | undefined): string {
  if (value === undefined || value.length === 0) throw new Error(configurationError)
  return value
}

export function readDatabaseConfiguration(env: NodeJS.ProcessEnv): DatabaseConfiguration {
  const portText = required(env.DB_PORT)
  if (!/^[0-9]+$/.test(portText)) throw new Error(configurationError)
  const port = Number(portText)
  if (!Number.isSafeInteger(port) || port < 1 || port > 65_535) throw new Error(configurationError)
