...
function environment(image, overrides = {}) {
  ...
  const dataSource = {
    isInitialized: false,
    initialize: async () => { dataSource.isInitialized = true },
    query: async (sql) => {
      calls.push(sql)
      if (sql === 'SHOW server_version') return [{ server_version: '18.6 (Debian 18.6-1.pgdg13+2)' }]
      assert.equal(sql, 'SHOW data_directory')
      return [{ data_directory: POSTGRES_DATA.pgdata }]
    },
    destroy: async () => { dataSource.isInitialized = false; calls.push('destroy') },
  }
  const runDocker = async (args) => {
    calls.push(args)
    if (args[0] === 'container') {
      assert.deepEqual(args, ['container', 'inspect', resources.containerName, '--format', '{{json .Image}} {{json .Platform}}'])
      return { stdout: `${JSON.stringify(overrides.imageId ?? image.imageId)} ${JSON.stringify(overrides.platform ?? 'linux')}\n` }
    }
    assert.deepEqual(args, ['exec', resources.containerName, 'uname', '-m'])
    return { stdout: `${overrides.architecture ?? (image.platform.includes('arm64') ? 'aarch64' : 'x86_64')}\n` }
  }
...
test('later server/container stage accepts the verified classic and containerd IDs on both platforms', async () => {
  for (const imageId of [configDigest, POSTGRES_INDEX_DIGEST]) {
    for (const platform of ['linux/arm64/v8', 'linux/amd64']) {
      ...
      assert.deepEqual(fixture.calls.slice(0, 3), ['SHOW server_version', 'SHOW data_directory', 'destroy'])
...
test('later server/container stage rejects a different image, platform or architecture', async () => {
  ...
  for (const overrides of [{ imageId: configDigest }, { platform: 'windows' }, { architecture: 'x86_64' }]) {
    const fixture = environment(image, overrides)
    await assert.rejects(assertServerAndContainer(resources, image, fixture.dependencies), { code: 'ERR_ASSERTION' })
    assert.equal(fixture.dataSource.isInitialized, false)
  }
})
