...
export const AuthSessionSchema = new EntitySchema<AuthSession>({
  ...
  foreignKeys: [
    {
      name: 'fk_auth_sessions_user', target: 'User',
      columnNames: ['userId'], referencedColumnNames: ['id'], onDelete: 'CASCADE',
    },
  ],
  indices: [
  ...
  checks: [
    ...
    { name: 'ck_auth_sessions_revoked_pair', expression: `("revoked_at" IS NULL) = ("revoked_reason" IS NULL)` },
    { name: 'ck_auth_sessions_revoked_time', expression: `"revoked_at" IS NULL OR "revoked_at" >= "created_at"` },
    { name: 'ck_auth_sessions_revoked_reason', expression: `"revoked_reason" IS NULL OR "revoked_reason" IN ('logout', 'refresh_reuse')` },
  ],
})
