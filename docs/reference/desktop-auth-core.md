...
기존 POSIX profile directory는 현재 user 소유·0700인지 확인하며 권한을 임의로 변경하지 않는다. Existing ancestor와 final direct parent도 root UID 또는 현재 user 소유이고 group/other write가 없어야 한다. 비디렉터리·symlink·권한/파일시스템 오류는 fail closed하고 `mkdir`·profile setter를 호출하지 않는다. `setPath` 뒤 즉시 `getPath('userData')`와 exact 비교한 적용 결과만 이후 구성에 사용하며, read-back·name·app identity setter 중 하나라도 실패하면 process를 nonzero로 종료한다. 상세 입력·순서와 보장 한계는 [Desktop Auth Profile Paths](desktop-auth-profile-paths.md)를 따른다. 이 검사는 문자열 provenance와 안정된 component의 순간 상태를 묶지만 inode 수명이나 검증 후 writable ancestor 교체 경쟁을 소유하지 않는다. macOS inherited ACL, Windows ACL/reparse point와 모든 filesystem의 전원 손실 durability는 trusted anchor와 native 검증 없이는 보장하지 않는다. 실제 배포 tuple의 profile pairwise 분리와 single-instance 독립성은 배포/native gate다.
