...
`LDB_AUTH_USER_DATA_PATH`는 설정 검증 뒤 lock 이전에 lstat한다. Native path segment의 trailing separator·`.`·`..` alias는 보정하지 않고 거절하며, 없는 trusted directory와 필요한 parent만 0700으로 만들고 POSIX directory handle 및 parent entry durability sync를 확인한다. 기존 POSIX directory는 현재 user 소유·0700인지 확인하며 권한을 임의로 변경하지 않는다. 비디렉터리·symlink·권한/파일시스템 오류는 fail closed한 뒤 `app.setPath('userData', ...)`를 호출하고, 그 다음 app identity를 적용한다. 이 tuple이 완전하지 않거나 profile 준비·적용이 실패하면 protocol lock·store·network를 활성화하지 않는다. 실제 배포 identity와 profile 값은 아직 결정하지 않는다.
