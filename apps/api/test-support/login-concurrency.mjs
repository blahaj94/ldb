...
import { assertCleared, counts, failure, fixture, proof, ready, row, started } from './login-database.mjs'
import { atExactTime, blockedBy, bounded, databaseNow, instrument, locked, settled, waitUntil } from './login-test-control.mjs'

const callbackQuery = (flow) => new URLSearchParams({ state: flow.state, code: 'fixture-provider-code' })
...
async function assertCallbackClaim(source) {
  ...
  try {
    ...
    await failure(() => f.service.callback('google', callbackQuery(flow), flow.cookie), 'LOGIN_REQUEST_INVALID')
...
async function assertSingleConsumer(source, kind) {
  ...
  await locked(source, 'auth_login_requests', id, async ({ pid, unlock }) => {
    const pids = [], observed = Promise.withResolvers()
    ...
    try {
      ...
      assert.equal(results.filter((result) => result.error?.code === (kind === 'exchange' ? 'LOGIN_EXCHANGE_INVALID' : 'LOGIN_REQUEST_INVALID')).length, 1)
    } finally { restore() }
  })
...
async function assertFreshAfterWait(source, table) {
  ...
  await source.query('UPDATE auth_login_requests SET code_expires_at=$2 WHERE id=$1', [flow.request.requestId, deadline])
...
async function assertExactExpiration(source) {
  ...
  await atExactTime(source, (await row(source, request.requestId)).expires_at, () =>
    failure(() => f.service.authorize(new URL(request.browserUrl).searchParams.get('ticket')), 'LOGIN_REQUEST_INVALID'))
  ...
  await atExactTime(source, (await row(source, browser.request.requestId)).expires_at, () =>
    failure(() => f.service.callback('google', callbackQuery(browser), browser.cookie), 'LOGIN_REQUEST_INVALID'))
  ...
  for (const field of ['code_expires_at', 'expires_at']) {
    ...
    await atExactTime(source, stored[field], () => failure(() => f.service.exchange(flow.exchange), 'LOGIN_EXCHANGE_INVALID'))
    assertCleared(await row(source, flow.request.requestId), 'failed')
    assert.deepEqual(await counts(source), before)
  }
}
...
async function assertCallbackDeadline(source) {
  const startedAt = Date.now(), entered = Promise.withResolvers(), release = Promise.withResolvers()
  let signal, calls = 0
  const f = await fixture(source, { verifyProvider: async (input) => {
    calls++
    signal = input.signal
    entered.resolve()
    await release.promise
...
async function assertCallbackExpiryAndCompletionTime(source) {
  ...
  await source.query('UPDATE auth_login_requests SET created_at=$2, expires_at=$3 WHERE id=$1', [flow.request.requestId, new Date(time.getTime() - 600_000), time])
  release.resolve()
  ...
  await source.query('UPDATE auth_login_requests SET created_at=$2, expires_at=$3 WHERE id=$1', [short.request.requestId, new Date(expiry.getTime() - 600_000), expiry])
...
async function assertTwoIdentityExchanges(source) {
  ...
  const f = await fixture(source, { issueAccessJwt: async (input) => {
    if (++signCalls === 1) { entered.resolve(); await release.promise }
    return signer(input)
  } })
  ...
  try {
    ...
    assert.deepEqual(await counts(source), { users: before.users + 1, sessions: before.sessions + 2, refresh: before.refresh + 2 })
  } finally { release.resolve(); await Promise.all([first, second]); restore() }
}
...
export async function assertLoginConcurrency(source, mark) {
  const cases = [
    ...
    ['OAuth row wait crosses code deadline', () => assertFreshAfterWait(source, 'auth_login_requests')],
