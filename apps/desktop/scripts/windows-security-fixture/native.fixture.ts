...
import { cleanupFixture, createFixtureRoot, fixturePath } from './isolation'
...
it('observes synthetic Win32 files without enabling product capabilities', async () => {
  ...
  const inspectReparse = (path: string): boolean => {
    ...
    return inspection === 'reparse'
  }
  const fixture = await createFixtureRoot(resolve(tmpdir()), { inspectReparse })
  const nativeAdapter = createWindowsCredentialNative()
  let stage = 'setup'
  let failure = false
  let directoryFlush = 'not-observed'
  let cleanup = 'cleanup-incomplete'
  let listedBytes = 0
  ...
  try {
    ...
    listedBytes = names.reduce((bytes, name) => bytes + 68 + Buffer.byteLength(name, 'utf16le'), 0)
    ...
    // Assertion errors can contain paths or native inputs. Report only fixed case IDs.
    failure = true
  } finally {
    const resourcesReleased = observed.releaseRemaining()
    cleanup = await cleanupFixture(fixture, { resourcesReleased })
    console.log(
      JSON.stringify({
