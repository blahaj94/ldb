...
import { searchFixture, withSearchApp, searchRequest, expectSearchError, snapshot, observeSearchRunners, waitFor } from './character-search-fixtures.mjs'
...
async function databaseFailure(source, phase, applied) {
  ...
  const createQueryRunner = observeSearchRunners({
    query: async ({ sql, run }) => {
      ...
      if (shouldFail) throw new Error('synthetic private SQL credential canary')
      return run()
    },
    commit: async (runner, commit) => {
      const shouldFail = fail && phase === 'commit'
      if (!shouldFail) return commit()
  ...
  process.stdout.write = (chunk) => { output.stdout += chunk.toString(); return true }
  process.stderr.write = (chunk) => { output.stderr += chunk.toString(); return true }
  try {
    await withSearchApp(f, async ({ base, calls }) => {
      const body = await expectSearchError(await searchRequest(base, f), 500, 'INTERNAL_SERVER_ERROR')
      ...
      fail = false
      for (let count = 0; count < 10; count += 1) assert.equal((await searchRequest(base, f)).status, 200)
  ...
  for (const secret of [f.token.accessToken, f.initial.refreshToken, f.identity.subject, f.initial.user.id,
    source.options.password, 'synthetic-search-key', 'synthetic private']) {
...
async function upstreamRetention(source, kind) {
  ...
  await withSearchApp(f, async ({ base, calls, upstream }) => {
    ...
    if (isTimeout) {
      ...
      upstream.body = { rows: [{ characterId: 'x', characterName: 'ab', serverId: 'cain', fame: '0' }] }
    }
    ...
    upstream.body = { rows: [] }
    for (let count = 1; count < 10; count += 1) assert.equal((await searchRequest(base, f)).status, 200)
    await expectSearchError(await searchRequest(base, f), 429, 'SEARCH_RATE_LIMITED')
    assert.equal(calls.length, 10)
  })
}
...
export async function assertSearchFailures(source, mark) {
  const cases = [
    ['connection refused remains search 500 and does not consume quota', () => refusedConnection(source)],
    ...['read', 'write'].map((phase) => [`${phase} failure leaves database and quota unchanged`, () => databaseFailure(source, phase, false)]),
    ...[false, true].map((applied) => [`unknown commit acknowledgement applied=${applied} stays 500 and unreserved`, () => databaseFailure(source, 'commit', applied)]),
    ...['empty', 'invalid-candidate', 'timeout'].map((kind) => [`${kind} upstream result retains activity and reservation`, () => upstreamRetention(source, kind)]),
    ['existing adapter error priority remains sanitized JSON over authenticated HTTP', () => upstreamCodes(source)],
  ]
