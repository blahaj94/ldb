...
Workflow가 자체적으로 확인하는 policy는 linked Issue, Red-before-Green evidence, approximate logic budget이다. Rule 변경은 별도 approval comment 검사 없이 사용자의 merge로 승인·활성화한다. 결과는 하나의 advisory summary comment로 유지되며 merge를 차단하지 않는다.
