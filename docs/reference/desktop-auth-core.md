...
Ready 뒤에는 안내 완료→dependency 생성으로 coordinator를 만든다. 안내가 완료되지 않아 runtime을 만들 수 없으면 protocol ingress를 폐기하되 적용된 profile의 single-instance ownership은 유지한 비인증 window로 전환한다. 정상적인 저장소·network 복원 실패는 `storageBlocked` 또는 `restorePaused` 같은 terminal/retry 상태로 resolve하고, 예상 밖 `start()` rejection만 ingress를 폐기한 뒤 nonzero 종료해 영구 `restoring`을 남기지 않는다. Window·auth/capture IPC·activate lifecycle을 먼저 연결한 뒤 `start()`로 restore를 시작하고, protocol return은 start 성공 뒤에만 전달한다.

Auth coordinator와 character search는 각각 독립된 runtime clock history를 사용하므로 한 consumer의 읽기가 다른 consumer의 역행 기준점을 소비하지 않는다. 관측된 clock 불연속이나 access 만료는 해당 access generation의 신뢰 상실로 고정하며 다음 정상 clock read만으로 같은 access를 되살리지 않는다. 각 runtime clock은 생성 시 실제 wall/monotonic 값을 기준점으로 잡고 두 절대값 중 하나가 역행한 경우만 현재 불연속으로 감지한다. Wall과 monotonic의 경과 차이, 큰 순방향 불연속, suspend를 판정할 정책·임계값은 정의하지 않았으므로 이 경우의 local access 적격성은 아직 보장하지 않는다. 실제 API/provider, OS protocol registry, safeStorage·file durability와 packaged native 성공도 별도 검증 범위다.
...
| `apps/desktop/src/backend/auth/runtime-effects.ts`       | 적용된 설정 tuple로 auth HTTP, macOS credential store, browser, consumer별 runtime clock, entropy와 저장소 접근 전 안내를 구성 |
...
Unit test는 Browser·HTTP·clock과 credential/marker 상태를 가진 store fake를 제어해 PKCE, URL, response stream, pending 취소·만료, duplicate/stale 복귀, commit 전 비공개, marker 결과와 재시작 recovery, restore와 `GET /me`, refresh single-flight, logout 경합과 snapshot 비노출을 확인한다. 제품 composition test는 설정 누락/오류의 무활성화, profile 준비/부분 적용, owner/loser, 안내→store→start 순서, 예상 밖 start rejection, packaged file document의 close→activate 재연결과 capture permission 경계를 합성 Electron double로 확인한다. Actual ingress module도 같은 double에 연결해 protocol listener와 일반 second-instance listener의 등록 순서, callback 1회와 window 예외 회수를 함께 검사한다. Runtime effects와 실제 macOS credential adapter의 non-macOS 거절을 함께 실행해 safeStorage·file·network side effect 없이 `storageBlocked`로 끝나는 것도 확인한다. Profile case alias는 host-dependent skip 대신 주입한 canonical filesystem seam으로 검사하며 실제 case-insensitive volume 관측은 native gate다. 실제 API `GET /me`, native credential store, OS protocol registry, Browser/provider, packaged app은 후속 gate다.
