...
async function authenticatedSearch(source) {
  ...
  await withSearchApp(f, async ({ base, calls, upstream }) => {
    upstream.body = { rows: [
      { characterId: 'candidate', characterName: '가나', serverId: 'cain', fame: 0, privateField: 'excluded' },
    ...
    assert.deepEqual(await response.json(), { rows: [
      { characterId: 'candidate', characterName: '가나', serverId: 'cain', serverName: '카인', fame: 0 },
      { characterId: 'future', characterName: 'other', serverId: 'future-server', serverName: null, fame: null },
    ] })
...
async function initialRejections(source) {
  ...
  await withSearchApp(f, async ({ base, calls }) => {
    await expectSearchError(await fetch(`${base}/characters?unknown=%FF`), 401, 'AUTHENTICATION_REQUIRED')
    await expectSearchError(await searchRequest(base, f, 'characterName=ab&limit=1&%6Cimit=2'), 400, 'INVALID_SEARCH_QUERY')
    await expectSearchError(await searchRequest(base, f, 'characterName=%FF'), 400, 'INVALID_SEARCH_QUERY')
    assert.deepEqual(calls, [])
  }, { apiKey: '' })
...
export async function assertCharacterSearchHttpIntegration(source, mark) {
  const cases = [
    ['real JWT and committed session activity precede loopback upstream projection', () => authenticatedSearch(source)],
    ['initial auth and raw query refusals preserve all database state and upstream count', () => initialRejections(source)],
  ]
