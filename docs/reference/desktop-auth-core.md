...
`LDB_AUTH_USER_DATA_PATH`는 설정 검증 뒤 lock 이전에 각 path component를 lstat한다. Native path segment의 trailing/repeated separator·`.`·`..` alias, Windows의 non-native `/` 구분자와 어느 위치의 symlink도 보정하거나 따라가지 않고 거절한다. 각 existing·created component와 missing component의 direct parent는 native canonical path를 다시 읽어 설정의 exact spelling과 같을 때만 사용하므로, case-insensitive filesystem에서 같은 directory를 가리키는 대소문자 alias도 거절한다. 없는 trusted directory와 필요한 parent만 0700으로 만들며, missing component의 direct parent를 mutation 전에 다시 확인한다. 새로 만들었거나 기존에 있던 최종 profile directory와 parent entry는 `setPath` 전에 POSIX directory handle로 durability sync한다. 기존 POSIX profile directory는 현재 user 소유·0700인지 확인하며 권한을 임의로 변경하지 않는다. 비디렉터리·symlink·권한/파일시스템 오류는 fail closed하고 profile setter를 호출하지 않는다. Profile 준비 오류는 auth composition을 비활성화하고, `setPath` 뒤 name 또는 app identity 적용까지 일부만 성공한 경우에는 다른 identity가 같은 profile을 사용하지 않도록 lock 전에 process를 nonzero로 종료한다. 실제 배포 identity와 profile 값은 아직 결정하지 않았으며, writable ancestor의 trusted anchor, macOS inherited ACL을 포함한 native private-root 판정과 전원 손실 durability는 별도 release gate다.
