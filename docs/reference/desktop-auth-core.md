...
Unit test는 Browser·HTTP·clock과 credential/marker 상태를 가진 store fake를 제어해 PKCE, URL, response stream, pending 취소·만료, duplicate/stale 복귀, commit 전 비공개, marker 결과와 재시작 recovery, restore와 `GET /me`, refresh single-flight, logout 경합과 snapshot 비노출을 확인한다. 제품 composition test는 설정 누락/오류의 무활성화, profile 준비/부분 적용, owner/loser, 안내→store→start 순서, notice 중 quit, 예상 밖 구성·start rejection, packaged file document의 close→activate 재연결과 capture permission 경계를 합성 Electron double로 확인한다. Actual ingress module도 같은 double에 연결해 단일 `second-instance` listener의 일반 실행·malformed·복수·pending 없는 callback 분류, 새 claim 1회와 window 예외 회수를 함께 검사한다. Runtime effects와 실제 macOS credential adapter를 함께 실행한 non-macOS test는 조기 `unavailable`→`storageBlocked`와 safeStorage·fetch 미호출을 확인한다. Filesystem 전역 미호출 전체를 계측한 증거는 아니다. Profile case alias는 주입한 canonical filesystem seam으로 검사하며 실제 case-insensitive volume 관측과 POSIX 전용 backslash 사례의 다른 OS 실행은 native gate다. 실제 API `GET /me`, native credential store, OS protocol registry, Browser/provider, packaged app은 후속 gate다.
