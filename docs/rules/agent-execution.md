...
- 댓글 작성 직전에 원격 PR의 최신 head, 라벨, 기존 요청과 리뷰 및 자동 요청 실행 상태를 확인합니다. 같은 head에 유효한 요청이 있거나 리뷰가 진행 중 또는 완료됐으면 중복 댓글 대신 기존 요청이나 결과를 연결합니다. 로컬 검증 대상과 원격 head가 다르면 해당 차이를 확인한 뒤 진행합니다. 실패한 요청을 진행 중이나 완료된 리뷰로 취급하지 않습니다.
- `@ldb-review` 라벨이 있는 동일 저장소의 review-ready PR은 기존 자동 요청을 우선합니다. `AI PR Review`의 signal 뒤 `AI PR Review Trusted`의 trigger까지 확인합니다. signal 성공이나 아직 댓글이 없다는 사실만으로 수동 요청하지 않으며, 해당 head의 자동 요청이 예정·대기·실행 중이거나 상태가 불명확하면 요청 처리를 기다리는 상태로 보고합니다. 저장소 owner가 작성한 `<!-- ldb-ai-review:codex:{headSha} -->` marker의 댓글과 head를 확인해 자동 요청 링크를 재사용합니다. 자동화의 원본은 [signal workflow](../../.github/workflows/ai-pr-review.yml), [trusted workflow](../../.github/workflows/ai-pr-review-trusted.yml)와 [trigger](../../scripts/pr-review/src/trigger.mjs)입니다.
- 수동 요청은 자동 요청 대상이 아니거나 해당 head의 자동 요청 실패·미실행이 확정되고, 요청을 게시할 작업이 더 이상 예정·대기·실행 중이 아님을 확인한 경우에만 합니다. 게시 직전 원격 head와 기존 댓글을 다시 확인합니다. 자동 요청을 재사용할 때 수정 요약과 검증 결과는 PR 본문이나 trigger 없는 답글에 남기며, 설명을 추가하려고 `@codex review` 댓글을 중복 작성하지 않습니다.
- 직접 작성하는 재검토 댓글은 `@codex review`로 시작하고 최신 PR head의 검토를 요청합니다. 본문에는 이번 수정 요약, 검증 결과와 한국어로 리뷰해 달라는 요청을 포함합니다. 커밋마다 요청하지 않고 한 차례의 리뷰 수정분을 모아 요청합니다. 작성과 게시 후 본문 확인은 [writing.md](writing.md)를 따릅니다.
