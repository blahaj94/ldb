...
export async function fixture(source, identity = { provider: 'google', subject: randomUUID() }) {
  ...
  const configuration = {
    issuer: 'https://refresh.test.invalid', audience: 'refresh-test',
    verificationKeys: [{ kid: 'fixture', publicKeyPem: keyPair.publicKey.export({ format: 'pem', type: 'spki' }) }],
    signingKey: { kid: 'fixture', privateKeyPem: keyPair.privateKey.export({ format: 'pem', type: 'pkcs8' }) },
  }
  ...
  return {
    initial, identity, deps, verifyJwt,
...
export async function stored(source, sessionId) {
  return {
    ...
    tokens: await source.query('SELECT * FROM auth_refresh_tokens WHERE session_id=$1 ORDER BY token_hash', [sessionId]),
  }
}

export async function setDeadline(source, sessionId, deadline) {
  const lastActiveAt = new Date(deadline.getTime() - idleSeconds * 1000)
  await source.query('UPDATE auth_sessions SET created_at=$2, last_active_at=$2 WHERE id=$1', [sessionId, lastActiveAt])
}
...
export async function withLock(source, kind, f, operation) {
  ...
  try {
    ...
    if (kind === 'users') {
      ...
      await runner.query('SELECT id FROM auth_sessions WHERE id=$1 FOR UPDATE', [f.initial.session.id])
    } else {
      assert.equal(kind, 'auth_refresh_tokens')
      await runner.query('SELECT token_hash FROM auth_refresh_tokens WHERE token_hash=$1 FOR UPDATE', [digest(f.initial.refreshToken)])
    }
...
export async function revoke(manager, sessionId, reason = 'logout') {
  ...
  await manager.query('UPDATE auth_sessions SET revoked_at=$2, revoked_reason=$3 WHERE id=$1', [sessionId, checkedAt, reason])
}
