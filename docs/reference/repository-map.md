...
- 공통 로그인: `src/auth/login/service.ts`가 요청·launch·callback claim·일회용 exchange를 처리하고 기존 identity-session과 `src/auth/access-jwt` issuer를 같은 transaction에 합성한다. `src/auth/login/http.ts`의 Nest factory는 실제 HTTP 경계다. 기본 AppModule/main의 자동 연결은 실제 adapter/등록/key 준비 전 gate로 남겨 두었다. Google 내부 연결점·검증 범위는 [`auth-login-development.md`](auth-login-development.md)를 참고한다.
