...
async function compileGenerated(directory) {
  ...
  const { outputText } = ts.transpileModule(content, { compilerOptions: { module: ts.ModuleKind.ESNext } })
...
async function assertOrmRoundTrip(dataSource) {
  ...
  try {
    ...
    const user = { id: randomUUID(), provider: 'google', providerSubject: 'Opaque:CaseSensitive', nickname: '테스트', createdAt: now }
    await users.insert(user)
    assert.deepEqual(await users.findOneByOrFail({ id: user.id }), user)
    const session = { id: randomUUID(), userId: user.id, createdAt: now, lastActiveAt: now, revokedAt: null, revokedReason: null }
    await sessions.insert(session)
    assert.deepEqual(await sessions.findOneByOrFail({ id: session.id }), session)
    const token = { tokenHash: Buffer.alloc(32, 7), sessionId: session.id, issuedAt: now, consumedAt: null }
    await tokens.insert(token)
    assert.deepEqual(await tokens.findOneByOrFail({ tokenHash: token.tokenHash }), token)
    const id = randomUUID()
    await logins.insert({ id, purpose: 'login', provider: 'google', clientId: 'desktop', providerConfigVersion: 'test',
      returnTargetId: 'test', createdAt: now, expiresAt: new Date(now.getTime() + 60_000), status: 'failed' })
...
export async function assertSchemaFirst(configuration, mark) {
  ...
  let created = false
  try {
    const originalDefinitions = await withDataSource(createDatabaseDataSource, configuration, async (source) => {
    ...
    assert.match(await generateMigration('GeneratedAuth', sourceFactory, initialDirectory), /generated:/)
    assert.deepEqual(await withDataSource(sourceFactory, generatedConfiguration, databaseSnapshot), before)
    const Initial = await compileGenerated(initialDirectory)
    const initialFactory = () => new DataSource({ ...createDatabaseOptions(generatedConfiguration), migrations: [Initial] })
    ...
    const changedUser = new EntitySchema({ ...UserSchema.options, columns: {
      ...UserSchema.options.columns, migrationProbe: { name: 'migration_probe', type: 'text', nullable: true },
    } })
    const entities = authSchemas.map((schema) => schema === UserSchema ? changedUser : schema)
    const changedFactory = () => new DataSource({ ...createDatabaseOptions(generatedConfiguration), entities, migrations: [Initial] })
    ...
    await withDataSource(nextFactory, generatedConfiguration, async (source) => {
      ...
      await assertSchema(source, mark, [new Initial().name])
      await source.undoLastMigration()
      assert.deepEqual((await databaseSnapshot(source)).relations.map(({ name }) => name), ['typeorm_migrations'])
      assert.deepEqual(await source.query('SELECT name FROM typeorm_migrations'), [])
    })
  } finally {
    try {
      if (created) await withDataSource(createDatabaseDataSource, configuration, (source) =>
