...
export async function assertLoginHttpIntegration(source, mark) {
  ...
  try {
    ...
    const prepare = async () => {
      ...
      const launch = await fetch(`${base}${launchUrl.pathname}${launchUrl.search}`, { redirect: 'manual' })
      ...
      const callback = await fetch(`${base}/auth/callback/google?state=${providerUrl.searchParams.get('state')}&code=fixture-provider-code`, {
      ...
      assert(code)
      assert.doesNotMatch(html, /fixture-provider-code|accessToken|refreshToken|providerVerifier|<script/)
      assert.deepEqual(await counts(source), initialCounts)
      return { request, exchange: { requestId: request.requestId, clientId: 'desktop', code, codeVerifier: verifier } }
    }
    ...
    const restore = instrument(source, { commit: async (_runner, commit) => {
      await commit(); committed.resolve(); await release.promise
    } })
    let delivered = false
    const pending = settled(post('/auth/exchange', flow.exchange).then((response) => { delivered = true; return response }))
    try {
    ...
    assert.deepEqual(Object.keys(tokens).sort(), ['accessToken', 'accessTokenExpiresAt', 'isNewUser', 'refreshToken', 'sessionExpiresAt', 'tokenType', 'user'])
    ...
    const undoHook = instrument(source, { commit: async (_runner, commit) => { await commit(); throw new Error('fixture-secret SQL detail') } })
    let unavailable
    try { unavailable = await post('/auth/exchange', uncertain.exchange) } finally { undoHook() }
