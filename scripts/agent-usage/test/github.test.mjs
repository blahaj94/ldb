...
function apiMock({ pr = pullRequest(), prComments = [], issueComments = [] } = {}) {
  ...
  const call = (args, body) => {
    ...
    if (args.join(" ") === "api user") return { login: "author" };
    ...
    if (args.includes("PATCH")) return { html_url: "https://github.com/comment/updated" };
    if (args.includes("POST")) return { html_url: "https://github.com/comment/created" };
...
test("publishReport skips unmerged and fork PRs without writing", () => {
  for (const pr of [pullRequest({ state: "CLOSED", mergedAt: null }), pullRequest({ isCrossRepository: true })]) {
...
test("publishReport trusts latest snapshot only from PR author or repository owner", () => {
  ...
  const mock = apiMock({
    prComments: [
      ...
      { id: 2, body: valid.replace(HEAD, "b".repeat(40)), user: { login: "attacker" }, created_at: "2026-09-05T01:20:00Z" },
    ],
  });
...
test("publishReport posts unavailable without zeros for missing or stale snapshot", () => {
  for (const prComments of [
    [],
    [{ body: snapshotComment(snapshot({ headSha: "b".repeat(40) })), user: { login: "author" }, created_at: "2026-09-05T01:00:00Z" }],
  ]) {
...
test("publishReport posts observed partial counts but remains unavailable", () => {
  ...
  const mock = apiMock({
    prComments: [{ body: snapshotComment(partial), user: { login: "owner" }, created_at: "2026-09-05T01:00:00Z" }],
  });
  const result = publishReport(REPOSITORY, 35, mock.call);
  assert.equal(result.status, "unavailable");
  const post = mock.calls.find(({ args }) => args.includes("POST"));
...
test("publishReport updates only its own per-PR marker on repeat", () => {
  ...
  const mock = apiMock({
    prComments: [{ body: snapshotComment(snapshot()), user: { login: "author" }, created_at: "2026-09-05T01:00:00Z" }],
    issueComments: [
      { id: 8, body: existing.body, user: { login: "someone" } },
      existing,
    ],
  });
  assert.equal(publishReport(REPOSITORY, 35, mock.call).status, "published");
...
test("publishReport targets every linked same-repository issue only", () => {
  const pr = pullRequest({
    closingIssuesReferences: [
      ...pullRequest().closingIssuesReferences,
      { number: 36, repository: { name: "repo", owner: { login: "owner" } }, url: "https://github.com/owner/repo/issues/36" },
      { number: 7, repository: { name: "other", owner: { login: "owner" } }, url: "https://github.com/owner/other/issues/7" },
    ],
  });
  const mock = apiMock({
    pr,
