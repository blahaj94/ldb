...
기존 전수 목록에 새 기준의 분류와 후속 PR을 연결한다. 최종 검증 담당에게 추가 미반영 항목을 전달하며, 진행 중 작업의 파일 소유권을 가져오지 않는다. 구현은 기존 작업 단위와 승인된 실행 계약을 따라 나누고, 한 대표 함수 수정이나 검색 결과만으로 전수 완료를 보고하지 않는다.

검증은 [기존 전수 완료 기준](#전수-목록과-완료-기준)과 `testing.md`를 따른다. 독립 검사에서는 앞 조건이 false이고 뒤 조건이 true인 입력도 각 이름의 결과와 맞는지 확인한다. 의존 검사에서는 앞 조건이 false인 경로의 property·호출 미실행, 원래 순서·횟수와 오류 우선순위를 확인한다. 기존 테스트가 이를 고정하면 재사용하고 evidence가 부족한 범위만 보강한다. 사용자 merge와 최종 기준 revision의 재대조 전에는 기존 코드 교정을 완료로 표시하지 않는다.
