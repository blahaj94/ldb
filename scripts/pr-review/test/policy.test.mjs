...
test('does not add a separate approval check for Rule changes', () => {
  const report = buildPolicyReport(
    createInput({
  ...
  assert.equal(statusFor(report, 'rule_approval'), undefined)
  assert.equal(statusFor(report, 'linked_issue'), 'pass')
  assert.equal(statusFor(report, 'test_evidence'), 'skipped')
  assert.equal(statusFor(report, 'logic_budget'), 'pass')
})
