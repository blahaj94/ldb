...
async function eligibleRows(source, cleanup) {
  ...
  await source.query('UPDATE auth_sessions SET created_at=$2 WHERE id=$1', [
    active.initial.session.id,
    old
  ])
  await source.query(
    'UPDATE auth_refresh_tokens SET issued_at=$2, consumed_at=$2 WHERE token_hash=$1',
    [digest(active.initial.refreshToken), old]
  )
  ...
  assert.equal(
    (await stored(source, active.initial.session.id)).session.revoked_reason,
    'refresh_reuse'
  )
  await cleanup(source)
}
...
async function transactionFailure(source, cleanup, outcome) {
  ...
  const ordered = fixtures.toSorted((left, right) =>
    left.initial.session.id.localeCompare(right.initial.session.id)
  )
  let commits = 0
...
async function compiledCommand(source, configuration) {
  ...
  assert.deepEqual(
    { code: result.code, signal: result.signal, stdout: result.stdout, stderr: result.stderr },
    {
      code: 0,
      signal: null,
      stdout: 'Authentication cleanup completed\n',
      stderr: ''
    }
  )
  assert.deepEqual(await stored(source, f.initial.session.id), { session: undefined, tokens: [] })
  ...
  const failed = await command(
    ...
    {
      cwd: new URL('..', import.meta.url),
      env: { ...environment, DB_NAME: 'fixture_missing_database' }
    }
  )
...
export async function assertAuthenticationCleanup(source, configuration, mark) {
  ...
  const cases = [
    [
      'eligibility, retained history, exact request/session boundary and repetition',
      () => eligibleRows(source, cleanupAuthentication)
    ],
    [
      'partial prior commit with final rollback',
      () => transactionFailure(source, cleanupAuthentication, 'rolled-back')
    ],
    [
      'partial prior commit with unknown committed result',
      () => transactionFailure(source, cleanupAuthentication, 'committed')
    ],
    [
      'compiled explicit command and owned connection teardown',
      () => compiledCommand(source, configuration)
    ]
  ]
